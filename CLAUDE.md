...
- **Login:** append `/login` to any of the above. On Tom's explicit instruction (31/08/2026: "I want the login portal to be on page footer no where else"), the only public entry point is a **"Staff Login"** link in the global footer's links row (`views/partials/site-footer.ejs`, class `footer-link-staff`, styled bolder/brighter than the surrounding Privacy/Useful Thinking links so it reads as a functional control rather than more footer copy — `views/index.ejs`). It renders identically logged-in or logged-out, on every public page (the footer partial is shared across all CMS pages and the standalone check tools), desktop and mobile. **This is the real Arrington business login only** — it authenticates against the `users` table (nat/tom/client accounts) and is completely separate from the Scott demonstration's own `/scott/login` (`scott_portal_users` table, its own access model in `lib/scott/access.js`). Neither route touches the other; adding or restyling this footer link does not change Workspace access (still gated on its own three env-var checks, see AI Workspace section) or Scott's `page_access` gating.
...
- **The nav must sit above the open menu.** The nav is fixed with its own stacking context, so the hamburger's `z-index` only counts inside it; the menu script adds `menu-open` to `#nav` while the menu is open and `nav.menu-open { z-index: 106 }` lifts the bar (logo and X) above the menu (105). Without it the X is covered and the only way out is to tap a link, which was the case site-wide until 26/09/2026. Both copies (`site-chrome-styles`/`index.ejs` styles, `site-chrome-script`/`index.ejs` script) carry it; `test/mobileMenuClose.test.js`.
...
**Where an enquiry came from (added 11/09/2026).** Until this date no lead carried any acquisition data: `routes/leads.js` stored `signup_source` (the Google prefill flag) and nothing else, so the channel behind a real enquiry (the 4 September Cornwall Handyman lead, for one) could not be established. Now a nonced snippet at the top of both copies of the site chrome script (`views/index.ejs` and `views/partials/site-chrome-script.ejs`, the usual duplicated pair) captures, **once per browser session on the first page seen**, the landing page (path plus query), `document.referrer`, any `utm_source/medium/campaign/term/content` and `gclid`, into `sessionStorage.arrLeadAttribution`, and exposes `window.__leadAttribution()`. Every form that writes a lead sends it as `attribution`: the footer contact form, the gated PDF request, the Product Guide contact step, the Market Ready Test submit, the Owner Dependency Quiz email-results, and the Commercial Gaps Review start (which stores it on the review row, like `signup_source`, because its lead row is written later with no request in scope). Server side `lib/leadAttribution.js` `parseAttribution()` allowlists nine keys, strips HTML, caps lengths, requires the landing page to be a root-relative path and the referrer to be an `http(s)` URL, and drops anything else silently; the result is stored in `leads.attribution` (JSONB, default `'{}'`, added by an idempotent `ALTER` in `db/schema.sql`, plus the same column on `commercial_gaps_reviews`). `describeAttribution()` adds `Source: google / cpc / Leads-Search-1`, `Landing page:`, `Referrer:` and `Google Ads click id:` lines to every owner notification email; `attributionSummary()` gives the one-phrase form shown as `Source:` in the admin Leads panel (`GET /api/admin/leads` now returns `signup_source`, `attribution` and a server-derived `source_summary`) and appended as `[source: ...]` to the CRM contact event summary in `lib/crm/contacts.js`. `parseAttribution()` decodes `&amp;` back to `&` after stripping tags, because the stripper escapes the text it keeps; until 26/09/2026 every landing page or referrer with more than one query parameter was stored as `...&amp;utm_source=...` (gclid and utm fields themselves were unaffected). Rows from before then still carry the escaped form. Storage can be unavailable (private window, blocked site data): every access is wrapped and a form still submits with nothing attached. Rows from before this date simply have `{}`. Tests: `test/leadAttribution.test.js` (parser in both directions, summary precedence, email lines, plus a source scan asserting every form sends it and both chrome scripts capture it). Verified 11/09/2026 over real HTTP on a fresh local database: a hostile object (`https://` landing page, `javascript:` referrer, HTML in a utm) stores as `{}`/stripped, the admin API returns `source_summary`, and the CRM sync carries the source.
...
**"How did you hear about us?" on the contact form (19/09/2026).** Tom's ask: one optional question on the existing enquiry form, six options (Google search, Google advert, Recommended by someone, Social media, AI assistant such as ChatGPT, Other), a small optional box when someone picks Other, and the answer arriving with the rest of the enquiry. Nothing else on the form changed.

`lib/heardAbout.js` is pure and is the ONE place the options are written down: the form renders from it, the route validates against it, and the notification email and the admin Leads panel both take their wording from it, so a stored value and the label a person sees cannot drift apart. Adding or renaming an option is a single edit there.

**Two rules carry it, and both are about the answer still meaning something a year from now.** The selection is matched against the allowlist by exact id and anything else is stored as `''` (the map is null-prototype, so a crafted `constructor` or `toString` cannot resolve through `Object.prototype` — the same shape as workspace finding T3 and the Scott `ACTION_DOMAINS` fix). And **the free-text box belongs to Other alone**: a submission selecting "Google search" with text attached has the text dropped, because otherwise a counted answer would no longer mean what the dropdown says it means. The browser clears the box on change for the same reason, so the screen and the record tell one story.

It is genuinely optional at every layer: a skipped question stores `''`, prints no line in the email, shows nothing in the panel, and never blocks a submission.

**It no longer SAYS "optional", and it must not become required (19/09/2026).** Tom's question: *"Is making it optional a barrier that means people won't use it? Or could you have it optional without saying its optional or how many people will it put off if its mandatory?"* Both halves answered, and the second is the one that matters.

The word is gone from the prompt and the Other box. A dropdown resting on its own question already LOOKS unanswered and skippable, so the marker carried no information and only nudged. Phone and best day/time keep theirs, because there it answers a real hesitation ("do I have to give you my number?"); nobody hesitates over admitting they found us on Google, so the same reassurance buys nothing. The `aria-label` matches the visible prompt exactly, so a screen reader hears what a sighted visitor reads.

**Required was considered and rejected on three grounds, in order of weight:**

1. **A forced answer is a WORSE answer.** Someone who does not want to answer picks whatever clears the field, and that lands in the same column as a real answer with nothing to tell them apart. The dataset stops meaning anything, which defeats the point of having a countable column at all.
2. **`attribution` already records the channel on 100% of enquiries**, automatically, with no friction (landing page, referrer, `utm_*`, `gclid`). This question only adds what tracking CANNOT see: word of mouth, ChatGPT, someone who saw the van. Making the supplementary signal mandatory when the primary one is already complete is poor value for the cost.
3. **The arithmetic is one-sided at Arrington's volume.** An enquiry is worth £500 to £3,400 and they arrive in ones and twos. A skipped dropdown costs a row of analytics; an abandoned form costs a client, and you never learn it happened. **And it could never be measured either way**: at a handful of form enquiries a month, detecting even a large drop would take years, so this cannot be settled by testing it. That is itself the argument for the option whose downside is bounded and visible.

Published "each extra field costs N%" figures were deliberately not used to decide this: they are mostly marketing content rather than research, and none of them is about this business.

`test/heardAbout.test.js` pins both halves TOGETHER, because they pull against each other: drop the word but quietly add `required` and the form starts refusing enquiries, keep it optional but print the marker and the label invites a skip. Watched red three ways: the word returning, `required` arriving in its place, and the over-reach of stripping the marker from phone as well. Verified in a real browser by filling only name, email and message and pressing Send: it sent, the browser raised no validation bubble, and the row stored `''`.

**Deliberately separate from `attribution`.** That column is what the BROWSER observed (landing page, referrer, `utm_*`, `gclid`); this is what the PERSON says. The two disagreeing is a useful fact rather than a conflict, so the email prints them one above the other: an ad click the visitor remembers as a recommendation is worth knowing. Two columns (`heard_about VARCHAR(40)`, `heard_about_other VARCHAR(255)`) rather than one free-text field, so the answer stays countable; both added by standalone `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, which is the pattern that actually reaches an existing database.

**The options reach the view through `app.locals`, not the partial's arguments.** `views/partials/site-footer.ejs` is included from **fourteen** views; threading a new local through all fourteen is fourteen chances to miss one, and a missed one is not a cosmetic slip but an EJS render error, so a live page would 500. `app.locals.heardAboutOptions` reaches every render. The usual duplicated pairs still had to be edited by hand: the toggle script in `views/index.ejs` **and** `views/partials/site-chrome-script.ejs`, and the CSS in `views/index.ejs` **and** `views/partials/site-chrome-styles.ejs`. A test asserts all four, because a change landing in one of a pair means the feature silently does nothing on half the site.

**Tests:** `test/heardAbout.test.js`, 12. Four defects watched red on the right assertion: free text kept regardless of option, a plain-object map letting a prototype key through, the answer collected but never emailed, and the toggle added to only one of the duplicated pair. Verified over real HTTP against a freshly seeded throwaway database, and in a real browser: the box hidden at rest, appearing only on Other, cleared when the choice changes, and re-hidden after a successful send; the select matching the other fields to the pixel (516px wide, 46px tall against 44px); zero CSP violations; and mobile at 390px with no overflow. Five submissions through the live endpoint stored exactly what they should, including the smuggled-text and prototype-id cases being dropped, and the captured notification emails read `How they heard about us: Google advert` and `How they heard about us: Other: Met you at the Devon Chamber breakfast`. The upgrade path was proven separately by dropping both columns from a seeded database, inserting a pre-existing lead, and re-running the real seed: the columns come back and the old row defaults to empty.

**Pre-existing failure, measured not assumed:** `test/workspace/receivablesRetrieval.test.js` case 4 fails on this branch AND identically on unmodified `main`. It pins `NOW = 2026-09-16T09:00:00Z` and asserts the record renders as `fresh` against `stale_after_days: 1`, so it rotted three days after it was written and will fail every day from now on. Nothing to do with this change, and not fixed here because Tom asked for nothing else to be touched, but it is a permanently-red test that will mask a real failure in that suite.
...
**Replacing one of the four PDFs (worked example, Enactment Sheet, 14/09/2026).** Tom approved a new 4-page public Enactment Sheet replacing the old 2-page one. Three things move together and the middle one is the trap:

1. **The file.** `private/pdfs/enactment-sheet.pdf` is overwritten in place. Filename and route are deliberately unchanged, so every existing link and the `doc_N_file` content row keep working and no content migration is needed for the download itself.
2. **The page count is LIVE CMS COPY, not code.** `doc_N_meta` lives in the `content` table, and the documents seed inserts with `ON CONFLICT DO NOTHING`, so editing the default in `db/seed.js` changes a fresh database and **nothing on production**. Correcting an existing deployment needs a guarded migration (marker row + `UPDATE ... WHERE content = <old value>`, so a CMS edit always wins). The one for this change is `evidence.enactment_sheet_pages_2026-09-14`. It is scoped by finding whichever `doc_N_file` row points at `enactment-sheet.pdf` and deriving the `_meta` key from it, **not** by matching the value: Half-Time Team Talk already reads `PDF, 4 pages`, so a value-only match is one careless edit from rewriting the wrong document. It also only stamps its marker once it has found something to act on, because a fresh database has no documents section at all (that seed is guarded on a page Tom created by hand).
3. **The preview is the first page and goes stale silently.** `public/img/docs/enactment-sheet.jpg` still showed the old document, whose first page reads "Page 1 of 2" on its face, directly contradicting the new caption. Regenerated with the command above, but at `-scale-to-x 910 -scale-to-y -1` rather than `-r 110`, so it matches the 910px width of the other three previews; the new one is portrait, which also removes the old landscape outlier from the fanned montage.
