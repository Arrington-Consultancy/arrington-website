...
- **Deploy:** `railway up` from project root. Railway's own built-in auto-deploy on push to `main` remains configured but unreliable — the GitHub Action below is now the primary deploy path; manual `railway up` from a local checkout is a fine fallback/for out-of-band deploys.
- **GitHub Action (`.github/workflows/deploy.yml`, added 25/07/2026, verified working same day):** runs `railway up --service arrington-prototype --detach` on every push to `main`. Requires a `RAILWAY_TOKEN` secret in the repo's GitHub Actions settings (Settings → Secrets and variables → Actions) — must be a **project token** (Railway dashboard → the service → Settings → Tokens → create a token scoped to that service/environment), not a personal account token, so the Action needs no `railway link` step and can't target the wrong project. **Gotcha:** the `arrington-prototype` Railway project has both the web service and a Postgres addon attached, so `railway up` without `--service arrington-prototype` fails with "Multiple services found" (this is exactly what happened on the Action's first run — fixed by adding the explicit `--service` flag).
