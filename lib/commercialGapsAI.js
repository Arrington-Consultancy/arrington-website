...
function buildMockInterpretation({ name, company, location, transcript }) {
  ...
  return {
    ...
    useful_observation: `Several of your answers point back to the same underlying pattern: ${primary.label.toLowerCase()} and ${secondary.label.toLowerCase()} are connected. Fixing one without the other tends not to hold.`,
...
async function interpretCommercialGaps({ name, company, location, transcript }) {
  ...
  if (liveEnabled) {
    try {
      ...
      console.error('Commercial Gaps Review: live AI output failed schema validation, retrying once.', firstCheck.errors);
      const retry = await callAnthropic({ name, company, location, transcript }, firstCheck.errors);
      ...
      console.error('Commercial Gaps Review: live AI call failed, falling back to mock.', err.message);
    }
  }
