...
const { attributionSummary } = require('../leadAttribution');
...
async function syncFromLeads() {
  ...
  const { rows } = await db.query(
    `SELECT id, kind, name, email, phone, message, document, signup_source, attribution, created_at
     FROM leads
  ...
  for (const lead of rows) {
    ...
    contacts += 1;
    const source = attributionSummary(lead.attribution);
