...
async function getSiteShellData() {
  ...
  // Where to Start — second synthetic nav entry, placed after Evidence so
  // proof comes before prices. Same construction and reasoning as
  // renderPage() in server.js.
  ...
  const navPages = evidenceIndex === -1
    ? [...pagesWithOwnerCheck, whereToStartNavEntry]
    : [...pagesWithOwnerCheck.slice(0, evidenceIndex + 1), whereToStartNavEntry, ...pagesWithOwnerCheck.slice(evidenceIndex + 1)];
