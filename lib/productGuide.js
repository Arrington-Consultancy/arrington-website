...
// ------------------------------------------------------------
// The Product Guide is a NEW surface, so it quotes a figure only where the
// catalogue says that figure may be published (Tom, 14/09/2026 — see the
// publicPriceApproved comment in lib/whereToStartOffers.js).
//
// `pricePence` is therefore omitted entirely rather than zeroed or replaced:
// a 0 here would render as "£0" and read as free, which is a worse untruth
// than saying nothing. Callers must test for its presence, which is why
// `priceApproved` is always sent alongside rather than left to be inferred
// from a missing key.
//
// No substitute figure and no "from £X" is invented to fill the space. The
// CTA still goes to the offer's own page, so nothing is hidden from someone
// who wants the detail; the guide simply does not do the quoting.
function offerSummary(offerId) {
  ...
  const summary = {
    id: offer.id,
    name: offer.name,
    priceApproved: offer.publicPriceApproved === true,
    path: offer.path || (offer.id === 'conversation' ? '/where-to-start#conversation-offer' : '/where-to-start')
  };
  if (summary.priceApproved) summary.pricePence = offer.pricePence;
  return summary;
}
