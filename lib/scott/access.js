...
if (skipLoginRequested && IS_PUBLIC_SITE) {
  console.warn(`${SKIP_LOGIN_ENV}=true was set but this deploy IS the public site, so refusing to bypass the Scott demo login. This flag only ever works on a non-public deploy.`);
...
// True when this session is a fictional staff login. Kept here (rather
// than importing clearance.js) to avoid a require cycle: clearance.js does
// not depend on this file, and this check only needs the session shape.
function isPortalUserSession(req) {
  const u = req.session && req.session.scottPortalUser;
  return !!(u && u.personaId);
}
...
function requireScottPageAccess(req, res, next) {
  maybeAutoLoginForDemo(req)
    .then(() => {
      // A fictional staff login (scott_portal_users) is its own complete
      // identity for this demo area: it authenticated against a real
      // password, and its clearance is bound server-side. It deliberately
      // has no row in `users` and no page_access grant, because it is not
      // a real site account and must never hold real site capability.
      if (isPortalUserSession(req)) return next();
      ...
      return hasScottAccess(req.session.user).then((allowed) => {
        // Governance finding F8 (30/08/2026): rendering the 404 view with
        // an empty navigation and a hardcoded theme made a /scott path
        // measurably distinguishable from a path that never existed, by
        // response size alone. A denial that looks different from the
        // real thing is not a denial, so this goes through the site's own
        // 404 renderer like any other missing page.
        if (!allowed) return require('../render404').render404(req, res);
        next();
      });
    })
    .catch(next);
...
function requireScottApiAccess(req, res, next) {
  maybeAutoLoginForDemo(req)
    .then(() => {
      if (isPortalUserSession(req)) return next(); // see requireScottPageAccess
      if (!req.session.user) {
