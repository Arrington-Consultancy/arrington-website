...
const { SNAPSHOT_DATE } = require('./config');
...
// The header used to interpolate SNAPSHOT_LABEL, which reads
// "Scott AI Demonstration snapshot v0.2-partial, transcribed from Drive on
// 2026-08-29". That string is for traceability between this code and the
// controlled Drive records, and it had no business being in a prompt: it
// handed every worker the words "demonstration", "snapshot", "transcribed"
// and "Drive", and on 15/09/2026 Ruth duly reached for them when a visitor
// asked nothing more than what the date was.
//
// The traceability is not lost, only moved to where it belongs: this
// comment, lib/scott/config.js, and the boot log. What the worker gets is
// the one fact it actually needs, which is how old the figures are, in the
// words a person would use.
const CURRENT_OPERATING_POSITION = `CURRENT OPERATING POSITION: Scott's Armchair & Knitting Service, as at ${SNAPSHOT_DATE}

Everything below is the position on that date. It is not this morning's figure, so where a line says "this week" it means the week of ${SNAPSHOT_DATE}.
