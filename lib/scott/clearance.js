...
// ------------------------------------------------------------
// Session persona ("view the demo as"). See the block comment above
// PERSONAS for why this is a session-scoped view selector rather than a
// second set of authenticated accounts.
// ------------------------------------------------------------
const SESSION_KEY = 'scottPersonaId';

function getSessionPersonaId(req) {
  const id = req.session && req.session[SESSION_KEY];
  return isValidPersona(id) ? id : DEFAULT_PERSONA;
}

// Returns true on success, false if the id was invalid (session left
// unchanged) — callers must check this rather than assuming the switch
// took effect, so a bad id from a tampered request body cannot silently
// leave the session in an unvalidated state.
function setSessionPersonaId(req, id) {
