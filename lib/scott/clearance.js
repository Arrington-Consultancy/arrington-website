...
// friendly name, so the raw database slug is shown to a reader instead.
// test/scott/domainLabels.test.js asserts this map stays complete for
// every domain the dataset actually uses, as a record domain and as a
// fieldDomains entry. That sentence used to sit here describing a test
// that did not exist, which is how fifteen unlabelled domains reached a
// screen intended for a prospective client.
const DOMAIN_LABELS = {
  // Lead Finder candidates (15/09/2026). Owner only: prospecting evidence
  // names businesses that have not been approached and reasoning that is
  // explicitly inference, and neither is the office's or the workshop's to
  // read. Granted by no PERSONA_DOMAINS list, so only Scott's '*' reaches
  // it, and by no WORKER_DOMAINS list except Governance & Assurance's
  // universal read.
  commercial_prospecting: 'Prospecting candidates and their evidence',
  ...
  // Added 14/09/2026, and the count is the point. The Company Brain
  // rendered raw database slugs ('assets_ops', 'vehicle_status') in a list
  // where every other row read as a sentence. Checking the map against the
  // dataset rather than against what was visible on screen found thirteen
  // more, all of them narrow operational domains that only reach the page
  // for the people who hold them, which is why nobody had noticed. The
  // comment above this map claimed a test asserted completeness; no such
  // test existed, so one does now (test/scott/domainLabels.test.js).
  assets_ops: 'Equipment, vehicles and maintenance',
  vehicle_status: 'Vehicle condition and MOT',
  todays_routes: 'Today’s collection and delivery route',
  route_alerts: 'Route alerts for today',
  route_customer_contact: 'Customer contact details on the route',
  job_ref_route: 'Which job each stop belongs to',
  booked_window: 'Booked collection and delivery windows',
  stock_qty: 'Quantities on the bench',
  materials: 'Material usage and waste',
  equipment_authorised: 'Who is authorised on which equipment',
  leave_training: 'Leave, absence and training records',
  overtime_totals: 'Overtime hours and totals',
  complaints_ops: 'Complaints, operational view',
  debtor_risk_flag: 'Accounts flagged as a credit risk',
...
// Null-prototype, and read with a hasOwn guard below. Without it,
// personaCanAct('constructor') resolves through Object.prototype to a
// function, which is truthy, and a persona holding '*' is then told they
// may perform it. Found by a test rather than by reading, and it is the
// same prototype-chain shape governance finding T3 caught in the Workspace
// lane router: a lookup table that answers for a key nobody put in it.
const ACTION_DOMAINS = Object.assign(Object.create(null), {
  ...
  // Deciding a request to release a payment. Narrower than an ordinary
  // management decision on purpose: the approvals queue is the authorised
  // route for anything to do with money (see lib/scott/banking.js), so the
  // person approving one has to be somebody who can see the position it
  // would be paid from.
  writeback_payment_release: 'finance_full',

  // ------------------------------------------------------------
  // Banking & Accounting (01/09/2026)
  // ------------------------------------------------------------
  // Same derived rule as everything above: the right to act on a record
  // requires the clearance to see it. No new domain was invented for this
  // area, so it grants nobody anything they could not already see.
  //
  // The split is the demonstration. 07Q Clearance C covers "routine
  // invoice and payment status", which is the sales and purchase ledger:
  // Chloe raises the invoices and enters the bills. It does not cover the
  // bank, the accounts or the company's cash position, so categorising a
  // bank line, allocating a receipt and posting a journal all need
  // finance_full, which in practice means Scott. The person who raises the
  // invoices cannot see the account it will be paid into, and that is
  // segregation of duties working rather than being described.
  //
  // Recording a bill is deliberately NOT approving or paying it. Entering
  // a supplier invoice creates a liability; releasing money against it is
  // a separate act that this system refuses to perform at all.
  invoice_create: 'invoice_status',
  // Deliberately finance_full and not invoice_status, corrected after a
  // canary sweep of the rendered pages caught the consequence rather than
  // the intent. 07Q Clearance C covers "routine invoice and payment
  // status" and explicitly excludes "detailed supplier cost/margin", so
  // the SALES ledger is Chloe's and the PURCHASE ledger is not. The
  // purchase ledger also carries the trade creditors control account,
  // which is a statement of what the company owes: an aggregate she may
  // not see, and one that a screen showing her the individual bills would
  // have handed her anyway.
  bill_record: 'finance_full',
  bank_categorise: 'finance_full',
  receipt_allocate: 'finance_full',
  journal_post: 'finance_full',
  payment_request: 'finance_full'
});
...
function personaCanAct(personaId, action) {
  if (!Object.hasOwn(ACTION_DOMAINS, String(action))) return false;
...
//
//   2. A real site ADMIN/CONTENT user (Tom, nat), or a specific real
//      account named in IMPERSONATION_EXTRA_USERNAMES below, who has
//      explicitly entered demonstration mode. Only these accounts may
//      impersonate, and only via setImpersonatedPersona() below which
//      re-checks canImpersonate() itself rather than trusting the
//      caller to have checked.
//
//   3. One of those same accounts with no impersonation set: the owner
//      view (Scott Mercer, full clearance), matching the fact that an
//      invited demonstrator is looking at the whole fictional company.
//
// Anything else (no session, an ordinary 'client' viewer not named in
// IMPERSONATION_EXTRA_USERNAMES, an unrecognised shape) falls to the
// most restrictive real persona rather than the default owner view.
...
// Individual real accounts trusted to browse the Scott demo as ANY
// persona despite being 'client' role, for showing the clearance model
// itself to a specific invited viewer. Deliberately a narrow, named
// allowlist rather than widening IMPERSONATION_ROLES to include
// 'client' generally, which would hand every future invited demo
// viewer the owner view by default. Add a username here only on Tom's
// explicit instruction naming that person (added for 'will', 01/09/2026).
const IMPERSONATION_EXTRA_USERNAMES = ['will'];

function canImpersonate(req) {
  const u = req.session && req.session.user;
  if (!u) return false;
  if (IMPERSONATION_ROLES.includes(u.role)) return true;
  return IMPERSONATION_EXTRA_USERNAMES.includes(String(u.username || '').toLowerCase());
}
