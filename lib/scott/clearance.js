...
// The narrowest real persona in the company: a workshop operative who
// holds his own van, his own jobs and the universal safety baseline, and
// nothing else. It is named here rather than written out at each use so
// the several places that fail closed cannot drift apart from one
// another. getEffectivePersonaId already failed closed to this value;
// personaDomains did not, which is the defect below.
const NARROWEST_PERSONA = 'mike_evans';

function isValidPersona(id) {
  return Object.prototype.hasOwnProperty.call(PERSONAS, id);
}

// Identity only: the name and role rendered in the sidebar. It carries no
// permission, and it deliberately keeps the owner as its fallback,
// because falling back to a narrower NAME would put the wrong person's
// name on the screen without changing what that screen may show.
//
// The lookup is hasOwnProperty-guarded all the same: a plain object
// answers `constructor` and `toString` from its prototype, so an
// unrecognised id could previously return the Object function to a view
// expecting a persona. Not reachable either, for the same reason as
// personaDomains above, and corrected in the same pass rather than left
...
//
// This used to fall back to DEFAULT_PERSONA, which is Scott Mercer, who
// holds '*' and therefore sees everything in the company. So an
// unrecognised persona id resolved to the OWNER view rather than to the
// narrowest one: the exact inversion of the rule 07Q states and the rule
// getEffectivePersonaId already applied thirty lines below, with its own
// comment saying "fail closed to the narrowest real persona rather than
// the owner".
//
// Found by the fourteenth Arrington Workspace reviewer while hunting a
// related class elsewhere, and reported as a latent concern rather than
// a finding, because it is not reachable: every call site passes a
// persona from a closed set, either getEffectivePersonaId's own
// validated output or a static ownership register that a test asserts
// holds only real personas. Nothing user-supplied reaches it.
//
// Corrected anyway, on Tom's instruction of 31/08/2026, because a known
// fail-open must not be carried silently into later Scott work. A
// control that is safe only because of who happens to call it is one
// refactor away from not being safe at all.
function personaDomains(personaId) {
  const persona = isValidPersona(personaId) ? personaId : NARROWEST_PERSONA;
  return Object.prototype.hasOwnProperty.call(PERSONA_DOMAINS, persona)
    ? PERSONA_DOMAINS[persona]
    : [];
}
