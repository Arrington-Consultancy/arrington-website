...
function allDeepFactRecords() {
  const arrays = [
    deepFacts.SERVICE_ECONOMICS, deepFacts.CURRENT_JOBS, deepFacts.WIP_AGEING_ALERTS,
    deepFacts.QUALITY_QUEUE, deepFacts.OPEN_PURCHASE_ORDERS, deepFacts.SUPPLIER_DIRECTORY,
    deepFacts.STOCK_SNAPSHOT, deepFacts.STAFF, deepFacts.HR_CURRENT_ISSUES,
    deepFacts.COST_OPPORTUNITIES
  ];
  const singles = [
    deepFacts.CORPORATE_PROFILE, deepFacts.DIRECTOR_POSITION, deepFacts.TAX_POSITION,
    deepFacts.FINANCE_SUMMARY, deepFacts.QUALITY_KPIS, deepFacts.CAPACITY_NOTE,
    ...deepFacts.BORROWING_SCHEDULE
  ];
  return [...arrays.flat(), ...singles];
}
...
function formatDeepFactsBlock(personaId, workerId) {
  const visible = clearance.filterByClearance(personaId, workerId, allDeepFactRecords());
...
module.exports = { extractEntities, buildContext, JOB_REF_PATTERN, formatDeepFactsBlock };
