...
// Evolving fictional business memory (31/08/2026 approved design change).
// Answering rule step 2: before a worker may generate a new fact, it must
// be shown any already-established fact that might answer the same
// question. clearance-gated exactly like everything else here — a fact's
// domain is checked with the same isDomainVisible() call as any other
// deep-brain record, so this never surfaces something the persona/worker
// pair could not otherwise see. Returns '' (no block) when nothing
// relevant exists or nothing relevant is visible, same convention as
// every other formatter in this file.
async function formatMemoryFactsBlock(personaId, workerId, message) {
  const facts = await memory.findRelevantFacts(personaId, workerId, message);
  if (!facts.length) return '';
  const lines = facts.map((f) => `- [${f.domain}] Q: ${f.canonicalQuestion} -> A: ${f.answer} (established ${new Date(f.createdAt).toISOString().slice(0, 10)} by ${f.createdByWorkerId}, fictional AI-created memory, fact id ${f.id})`);
  return `PREVIOUSLY ESTABLISHED FICTIONAL COMPANY MEMORY (created earlier in this demonstration, not part of the original controlled evidence: if one of these answers the current question, use it verbatim and do NOT propose a new memoryFact for it; model variation is never a reason to restate it differently)\n${lines.join('\n')}`;
}
