...
function formatDeepFactsBlock(personaId, workerId) {
  ...
  //
  // NO LEVEL CAP (15/09/2026). This function used to narrow the record set
  // by the progression level as well, so Level 1 reached one source class.
  // Tom's settled model is "same brain, more tools": the level sizes the
  // WORKSPACE, and clearance alone decides what a person may know. Moving
  // Scott from Level 4 to Level 1 must not make authorised knowledge
  // disappear, so the only filter left here is the clearance one, which is
  // the filter that was always doing the security work.
...
// TWO legs, ANDed, narrowest wins: the human's clearance and the worker's
// own permission. That is 07Q/05A and it is the whole rule.
//
// There was briefly a third leg here, the progression level (15/09/2026,
// removed the same day). It is gone on Tom's instruction because it
// answered the wrong question: a level says how big the workspace is, not
// what a person is entitled to know. Knowing something and having the tool
// to act on it are now decided in different places — this function, and
// lib/scott/capabilityRegistry.js — and neither stands in for the other.
