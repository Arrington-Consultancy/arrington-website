...
function formatDeepFactsBlock(personaId, workerId, level) {
  ...
  // The level cap is applied FIRST and separately, so a record the level
  // excludes never reaches the redactor at all. Above Level 1 this filter
  // passes everything, so the behaviour is byte-identical to what it was.
  const cap = progression.contextDomainCap(level === undefined ? progression.MAX_LEVEL : level);
  const source = cap === null
    ? allDeepFactRecords()
    : allDeepFactRecords().filter((r) => r && cap.includes(r.domain));
  const visible = clearance.filterAndRedact(personaId, workerId, source);
...
// THREE legs now, not two, and they are ANDed: the human's clearance, the
// worker's own permission, and the progression level the visitor is at.
// Narrowest wins, exactly as before — the level can only ever take a
// source class away, never add one, because above Level 1 it caps nothing
// at all (lib/scott/progression.js). That is what makes Level 1's "here is
// what I cannot see" an honest statement about the system rather than a
// caption written next to it.
function visible(personaId, workerId, domain, level) {
  if (!progression.levelAllowsDomain(level === undefined ? progression.MAX_LEVEL : level, domain)) return false;
...
function formatJob(job, personaId, workerId, level) {
  if (!job) return '';
  if (!visible(personaId, workerId, CONTEXT_DOMAINS.job, level)) return '';
  const showPrice = visible(personaId, workerId, CONTEXT_DOMAINS.jobPrice, level);
...
function formatJobsList(jobs, heading, personaId, workerId, level) {
  if (!visible(personaId, workerId, CONTEXT_DOMAINS.job, level)) return '';
...
function formatCustomerHistory(history, personaId, workerId, level) {
  if (!history) return '';
  if (!visible(personaId, workerId, CONTEXT_DOMAINS.customer, level)) return '';
  const showJobs = visible(personaId, workerId, CONTEXT_DOMAINS.job, level);
  const showEnquiries = visible(personaId, workerId, CONTEXT_DOMAINS.enquiry, level);
  const showActivity = visible(personaId, workerId, CONTEXT_DOMAINS.activity, level);
...
function formatActivity(rows, heading, personaId, workerId, level) {
  if (!visible(personaId, workerId, CONTEXT_DOMAINS.activity, level)) return '';
...
async function buildContext(workerId, { message, entities, personaId, level }) {
  const blocks = [];
  ...
  // Defaults to the widest level for exactly the same reason the persona
  // defaults to the owner: every caller that predates the progression
  // (the public lead form's fire-and-forget draft, and the existing
  // tests) must keep behaving as it did. The level only ever narrows, so
  // "no level supplied" has to mean "narrow nothing".
  const lvl = level === undefined || level === null ? progression.MAX_LEVEL : progression.normaliseLevel(level);
  const can = (domain) => visible(persona, workerId, domain, lvl);

  blocks.push(formatDeepFactsBlock(persona, workerId, lvl));

  if (entities.job) blocks.push(formatJob(entities.job, persona, workerId, lvl));
  if (entities.customer) blocks.push(formatCustomerHistory(entities.customer, persona, workerId, lvl));
  ...
  if (workerId === 'operations') {
    if (!entities.job && can(CONTEXT_DOMAINS.job)) {
      ...
      blocks.push(formatJobsList(atRisk, 'Jobs currently flagged at risk', persona, workerId, lvl));
      blocks.push(formatJobsList(dueSoon, 'Jobs due within 7 days', persona, workerId, lvl));
    }
    ...
    if (can(CONTEXT_DOMAINS.activity)) {
      blocks.push(formatActivity(await repo.getRecentActivity(12), 'Most recent demonstration activity', persona, workerId, lvl));
    }
    if (!entities.job && can(CONTEXT_DOMAINS.job)) {
      const refMatch = String(message || '').match(JOB_REF_PATTERN);
      if (refMatch) blocks.push(formatJob(await repo.getJobByRef(refMatch[0].toUpperCase()), persona, workerId, lvl));
    }
    ...
    if (can(CONTEXT_DOMAINS.activity)) {
      blocks.push(formatActivity(await repo.getRecentActivity(15), 'Most recent demonstration activity', persona, workerId, lvl));
    }
  }
