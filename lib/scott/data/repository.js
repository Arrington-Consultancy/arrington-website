...
async function createConversation(userId, title, { jobId, enquiryId } = {}) {
  const { rows } = await db.query(
    ...
    [userId, title || 'New conversation', jobId || null, enquiryId || null]
  );
  return rows[0];
}

// Finds the most recent conversation matching a given context (a specific
// job, a specific enquiry, or — when both are omitted — the general
// dashboard chat with neither). Used to give a returning visitor
// continuity instead of losing their conversation on every page reload.
async function getLatestConversation(userId, { jobId, enquiryId } = {}) {
  const { rows } = await db.query(
    `SELECT * FROM scott_conversations
     WHERE user_id = $1 AND related_job_id IS NOT DISTINCT FROM $2 AND related_enquiry_id IS NOT DISTINCT FROM $3
     ORDER BY updated_at DESC LIMIT 1`,
    [userId, jobId || null, enquiryId || null]
  );
  return rows[0] || null;
}
...
async function getConversation(id, userId) {
  const { rows } = await db.query(
    'SELECT * FROM scott_conversations WHERE id = $1 AND user_id = $2',
    [id, userId]
  );
  return rows[0] || null;
}
