...
// Admits a candidate the autofill rule cleared, without a person.
//
// A separate function from decideBrainCandidate rather than a flag on it,
// so the two are distinguishable in the code and in the register: this one
// writes 'automatic' as the decider and can never be reached from the
// approval route, and decideBrainCandidate always means a human clicked.
// The same status guard applies, so an already-decided row is untouched
// and the null return says so.
async function autofillBrainCandidate(id, { reason } = {}) {
  const { rows } = await db.query(
    `UPDATE scott_brain_candidates
        SET status = 'approved', decision_note = $1, decided_by_name = 'automatic',
            decided_by_persona_id = '', decided_at = NOW()
      WHERE id = $2 AND status = 'pending'
      RETURNING *`,
    [`Admitted without review: ${String(reason || '').slice(0, 1900)}`, id]
  );
  return rows[0] || null;
}
