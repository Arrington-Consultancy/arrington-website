...
// Closing a gap by logic rather than by a person (13/09/2026). The decision
// is made by the pure lib/scott/gapClosure.js and written verbatim here, on
// the same principle as createBrainGap: decide once in a function that can
// be tested without a database, store what was decided.
//
// source_corrected is left FALSE and never takes a parameter. That column
// is a human's explicit statement that they corrected a controlled record,
// and no code can make that true. 'automatic' as the closer is what lets
// the register tell the two apart. The status guard means a gap a person
// has already closed is untouched, and the null return says so.
async function closeBrainGapAutomatically(id, { status, note } = {}) {
  if (!['resolved', 'dismissed'].includes(status)) {
    throw new Error(`unknown automatic gap closure status: ${status}`);
  }
  const { rows } = await db.query(
    `UPDATE scott_brain_gaps
        SET status = $1, source_corrected = false, resolution_note = $2,
            resolved_by_name = 'automatic', resolved_at = NOW()
      WHERE id = $3 AND status IN ('open','notified','awaiting_source')
      RETURNING *`,
    [status, String(note || '').slice(0, 2000), id]
  );
  return rows[0] || null;
}

// Gaps closed by code since a moment, for the briefing. A human closure is
// excluded by the closer name the write above always sets.
async function getGapsClosedAutomaticallySince(since) {
  const { rows } = await db.query(
    `SELECT * FROM scott_brain_gaps
      WHERE resolved_by_name = 'automatic'
...
// Every proposal attached to any of these gaps, so the closure rule can see
// what was proposed for each without a query per gap.
async function getBrainCandidatesForGaps(gapIds = []) {
  const ids = gapIds.map((n) => parseInt(n, 10)).filter(Number.isInteger);
  if (!ids.length) return [];
  const { rows } = await db.query(
    'SELECT * FROM scott_brain_candidates WHERE gap_id = ANY($1::int[]) ORDER BY id',
    [ids]
  );
  return rows;
}
  ...
  const { rows } = await db.query(
    `INSERT INTO scott_brain_candidates
       (gap_id, conversation_id, domain, fact_key, fact_value, source_label,
        ...
        estimated, basis)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10::jsonb,'pending',$11,$12) RETURNING *`,
    [c.gapId || meta.gapId || null, meta.conversationId || null,
     ...
     c.estimated === true, String(c.basis || '').slice(0, 2000)]
  );
  return rows[0];
}

// Settles a candidate by code, without a person (13/09/2026). The outcome
// comes from brainCandidates.settleCandidate and is one of admit, reject or
// redundant; the status written is approved, rejected or superseded
// respectively, always with 'automatic' as the decider so a code decision
// and a human decision are distinguishable in the register. The status
// guard means an already-decided row is untouched and the null return says
// so. Writes only scott_brain_candidates: nothing here can reach any real
// Arrington table.
const SETTLEMENT_STATUS = Object.freeze({ admit: 'approved', reject: 'rejected', redundant: 'superseded' });
const SETTLEMENT_NOTE = Object.freeze({ admit: 'Admitted automatically', reject: 'Rejected automatically', redundant: 'Not added, already held' });

async function settleBrainCandidate(id, { outcome, reason } = {}) {
  const status = SETTLEMENT_STATUS[outcome];
  if (!status) throw new Error(`unknown settlement outcome: ${outcome}`);
  const { rows } = await db.query(
    `UPDATE scott_brain_candidates
        SET status = $1, decision_note = $2, decided_by_name = 'automatic',
            decided_by_persona_id = '', decided_at = NOW()
      WHERE id = $3 AND status = 'pending'
      RETURNING *`,
    [status, `${SETTLEMENT_NOTE[outcome]}: ${String(reason || '').slice(0, 1900)}`, id]
  );
  return rows[0] || null;
}

// Kept for the seed's authored-record path, which admits facts it has
// already checked itself. Same write as an admit settlement.
async function autofillBrainCandidate(id, { reason } = {}) {
  return settleBrainCandidate(id, { outcome: 'admit', reason });
}

// A person removing a fact the company learned on its own. The one human
// action left in this area, and it runs in the opposite direction from the
// old approval: it takes a fact OUT. Status goes to rejected (the CHECK
// constraint's vocabulary) with a note that names the person and says
// retracted, so the register can tell a retraction from an automatic
// rejection. Only an approved row can be retracted.
async function retractBrainCandidate(id, { note, decider } = {}) {
  const { realUserId = null, personaId = '', displayName = '' } = decider || {};
  const { rows } = await db.query(
    `UPDATE scott_brain_candidates
        SET status = 'rejected', decision_note = $1, decided_by_user_id = $2,
            decided_by_name = $3, decided_by_persona_id = $4, decided_at = NOW()
      WHERE id = $5 AND status = 'approved'
      RETURNING *`,
    [`Retracted by ${String(displayName || 'a person').slice(0, 120)}: ${String(note || '').slice(0, 1800)}`,
     realUserId, String(displayName).slice(0, 120), String(personaId).slice(0, 40), id]
  );
  return rows[0] || null;
}

// A person CHANGING a fact the company holds (13/09/2026). The one path
// that can replace a held fact, and it exists because the automatic rule
// deliberately has none: settlement always keeps the earlier fact, so
// without this the only way to change a figure would be to retract it and
// leave a hole.
//
// Nothing is overwritten. The old row moves to 'superseded' and points
// forward; a new approved row is written pointing back. Both stay readable,
// so "what did the company used to think, and who changed it" is answerable
// from the register rather than lost. One transaction, so the brain can
// never hold both versions or neither.
//
// Writes only scott_brain_candidates. There is no path from here to any
// real Arrington table.
async function supersedeBrainFact(id, { factValue, basis, note, decider, estimated } = {}) {
  const { realUserId = null, personaId = '', displayName = '' } = decider || {};
  const client = await db.pool.connect();
  try {
    await client.query('BEGIN');
    const current = await client.query(
      `SELECT * FROM scott_brain_candidates WHERE id = $1 AND status = 'approved' FOR UPDATE`,
      [id]
    );
    ...
    const value = String(factValue == null ? '' : factValue).trim();
    if (!value) {
      await client.query('ROLLBACK');
      throw new Error('a superseding fact must state a value');
    }
    const isEstimate = estimated === undefined ? old.estimated === true : estimated === true;
    const inserted = await client.query(
      `INSERT INTO scott_brain_candidates
         (gap_id, conversation_id, domain, fact_key, fact_value, source_label,
          proposed_by_worker_id, verdict, drift_flags, conflict_flags, status,
          decided_by_user_id, decided_by_name, decided_by_persona_id, decision_note,
          decided_at, estimated, basis, supersedes_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'admissible','[]'::jsonb,'[]'::jsonb,'approved',
               $8,$9,$10,$11,NOW(),$12,$13,$14)
       RETURNING *`,
      [old.gap_id, old.conversation_id, old.domain, old.fact_key, value,
       old.source_label, old.proposed_by_worker_id, realUserId,
       String(displayName || 'a person').slice(0, 120), String(personaId).slice(0, 40),
       `Corrected by ${String(displayName || 'a person').slice(0, 80)}: ${String(note || '').slice(0, 1700)}`,
       isEstimate, String(basis == null ? old.basis : basis).slice(0, 2000), old.id]
    );
    const next = inserted.rows[0];
    // RETURNING, and the returned row is what the caller is handed as
    // `previous`. Returning the row as it was READ would describe the fact
    // as still held a line after it stopped being, which is the class of
    // untruth this area keeps finding in itself.
    const replaced = await client.query(
      `UPDATE scott_brain_candidates
          SET status = 'superseded', superseded_by_id = $1,
              decision_note = $2, decided_by_user_id = $3, decided_by_name = $4,
              decided_by_persona_id = $5, decided_at = NOW()
        WHERE id = $6
      RETURNING *`,
      [next.id,
       `Superseded by #${next.id}, corrected by ${String(displayName || 'a person').slice(0, 80)}: ${String(note || '').slice(0, 1600)}`,
       realUserId, String(displayName || 'a person').slice(0, 120),
       String(personaId).slice(0, 40), old.id]
    );
    await client.query('COMMIT');
    return { previous: replaced.rows[0] || old, current: next };
  } catch (err) {
    try { await client.query('ROLLBACK'); } catch (e) { /* the original error is the one worth raising */ }
    throw err;
  } finally {
    client.release();
  }
...
// Supersessions since a moment, for the briefing's state-change section.
// Read from the NEW row, so each entry carries the value that now stands
// and names the row it replaced.
async function getBrainSupersessionsSince(since) {
  const { rows } = await db.query(
    `SELECT n.*, o.fact_value AS previous_value, o.estimated AS previous_estimated
       FROM scott_brain_candidates n
       JOIN scott_brain_candidates o ON o.id = n.supersedes_id
      WHERE n.supersedes_id IS NOT NULL
        AND ($1::timestamptz IS NULL OR n.decided_at > $1)
...
// The oversight view: what the company decided lately, in either direction.
async function getRecentBrainChanges({ limit = 50 } = {}) {
  const { rows } = await db.query(
...
// Rows the code rejected since a moment, for the briefing. Retractions are
// excluded here (they are a person's decision, reported separately) by the
// decider name the settlement always writes.
async function getBrainRejectionsSince(since) {
  const { rows } = await db.query(
    `SELECT * FROM scott_brain_candidates
      WHERE status = 'rejected' AND decided_by_name = 'automatic'
        AND ($1::timestamptz IS NULL OR decided_at > $1)
...
async function getBrainRetractionsSince(since) {
  const { rows } = await db.query(
    `SELECT * FROM scott_brain_candidates
      WHERE status = 'rejected' AND decided_by_name <> 'automatic'
        AND decision_note LIKE 'Retracted by %'
        AND ($1::timestamptz IS NULL OR decided_at > $1)
      ORDER BY decided_at ASC`,
    [since || null]
  );
...
// Should always be zero since 13/09/2026. A non-zero count is a system
// fault (a settlement that did not complete), which is what the briefing
// escalates.
async function countPendingBrainCandidates() {
  const { rows } = await db.query(`SELECT COUNT(*)::int AS n FROM scott_brain_candidates WHERE status = 'pending'`);
  return rows[0] ? rows[0].n : 0;
}

// System-level events since a moment, for the briefing's escalation block.
async function getScottSystemEventsSince(since, eventTypes = []) {
  if (!eventTypes.length) return [];
  const { rows } = await db.query(
    `SELECT event_type, summary, created_at FROM scott_activity
      WHERE event_type = ANY($1::text[]) AND ($2::timestamptz IS NULL OR created_at > $2)
      ORDER BY created_at ASC LIMIT 200`,
    [eventTypes, since || null]
  );
  return rows;
}
...
// Approved additions decided after a given moment, for the evolution
// briefing. Null `since` means everything, which is what the first
// briefing on a database wants.
async function getBrainFactsSince(since) {
  const { rows } = await db.query(
    `SELECT * FROM scott_brain_candidates
      WHERE status = 'approved' AND ($1::timestamptz IS NULL OR decided_at > $1)
      ORDER BY decided_at ASC`,
    [since || null]
  );
  return rows;
...
// When the last briefing actually went out. Read from the activity log
// rather than held in memory: a container restart would otherwise reset it
// and send a second copy of a briefing already delivered, and this app
// restarts on every deploy.
async function getLastDigestAt() {
  const { rows } = await db.query(
    `SELECT created_at FROM scott_activity
      WHERE event_type = 'brain_digest_sent'
      ORDER BY created_at DESC LIMIT 1`
  );
...
module.exports = {
  ...
  countOpenBrainGaps,
  resolveBrainGap,
  closeBrainGapAutomatically,
  getGapsClosedAutomaticallySince,
  getBrainCandidatesForGaps,
  createBrainCandidate,
  autofillBrainCandidate,
  settleBrainCandidate,
  retractBrainCandidate,
  supersedeBrainFact,
  getBrainSupersessionsSince,
  getRecentBrainChanges,
  getBrainRejectionsSince,
  getBrainRetractionsSince,
  countPendingBrainCandidates,
  getScottSystemEventsSince,
