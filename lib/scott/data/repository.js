...
async function getLatestConversation(userId, { jobId, enquiryId } = {}) {
  ...
  const { rows } = await db.query(
    scoped
      ...
      : `SELECT * FROM scott_conversations WHERE user_id = $3 AND related_job_id IS NULL AND related_enquiry_id IS NULL ORDER BY updated_at DESC LIMIT 1`,
    scoped ? [jobId || null, enquiryId || null] : [null, null, userId]
  );
  return rows[0] || null;
}
