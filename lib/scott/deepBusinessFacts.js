...
const BORROWING_SCHEDULE = [
  { domain: 'director_position', kind: 'overdraft_facility', lender: 'fictional bank', limitGbp: 15000, drawnGbp: 0, purpose: 'short-term working-capital contingency only' },
  {
    ...
    // Moved here 01/09/2026 from the banking module, whose figures were
    // removed because they contradicted 07A. This is not one of those
    // figures: it is a structural fact about the company that does not
    // move when a transaction is posted, and it is the single clearest
    // statement of the owner dependency the whole demonstration exists to
    // illustrate. It belongs on the borrowing it attaches to.
    personalGuaranteeGiven: true,
    personalGuaranteeBy: 'Scott Mercer',
    personalGuaranteeLimitGbp: 35000,
    keyPersonNote: 'The company\'s borrowing rests on one person\'s signature. If Scott stopped, the guarantee, the lending and the succession question all arrive at once, and no other director exists to carry them.'
  },
  {
...
// ------------------------------------------------------------
// 07L COLLECTION & DELIVERY ROUTE DATA (driver-facing, 2026-09-05)
// ------------------------------------------------------------
// Two-stop Friday route. Stop 1 is a collection of Priya Patel's
// armchair (SAKS-1041, booked collection window 09:00-11:00).
// Stop 2 is the return of Hannah Brooks' restored dining chairs
// (SAKS-1050, job completed, ready since 2026-09-03, promised
// return window 13:30-15:00). Van SAKS22V.
//
// Tagged with the driver-facing clearance domains so Mike Evans
// sees his own day, Operations sees it for scheduling, and nobody
// else does. Customer contact detail is route_customer_contact so
// it stays off the open job board.
const TODAYS_ROUTE = {
  domain: 'todays_routes',
  date: '2026-09-05',
  day: 'Friday',
  vehicle: 'SAKS22V',
  driver: 'Mike Evans',
  stops: [
    {
      ...
      address: '14 Meadow Close, Paignton, TQ3 2RJ',
      bookedWindow: '09:00 to 11:00',
      item: 'Victorian button-back armchair, collection for full restoration',
      handlingNote: 'Upholstered item, use furniture blankets. Customer has requested minimal disruption as she works from home.'
    },
    {
      stopNumber: 2,
      type: 'RETURN',
      ...
      address: '7 Harbour View Road, Brixham, TQ5 8LN',
      bookedWindow: '13:30 to 15:00',
      item: 'Set of four dining chairs, restoration complete',
      handlingNote: 'Return completed chairs. Job has been ready since 2026-09-03. Customer has been notified of delay.'
    }
  ],
  totalMiles: 38,
  estimatedDuration: '5.5 hours including stops'
};

const ROUTE_CUSTOMER_CONTACTS = {
  domain: 'route_customer_contact',
  contacts: [
    {
      jobRef: 'SAKS-1041',
      name: 'Priya Patel',
      mobile: '07741 882 341',
      address: '14 Meadow Close, Paignton, TQ3 2RJ',
      note: 'Prefers to be called before arrival. Work from home, so she will be in all morning.'
    },
    {
      jobRef: 'SAKS-1050',
      name: 'Hannah Brooks',
      mobile: '07823 664 019',
      address: '7 Harbour View Road, Brixham, TQ5 8LN',
      note: 'Has been waiting since Thursday. Call ahead as parking on Harbour View Road is limited.'
    }
  ]
};

const ROUTE_JOB_REFS = {
  domain: 'job_ref_route',
  mappings: [
    { stopNumber: 1, jobRef: 'SAKS-1041', type: 'COLLECTION', stage: 'COLLECTION BOOKED' },
    { stopNumber: 2, jobRef: 'SAKS-1050', type: 'RETURN', stage: 'READY FOR RETURN', readySince: '2026-09-03', promisedDate: '2026-09-03', daysLate: 2 }
  ]
};

const ROUTE_BOOKED_WINDOWS = {
  domain: 'booked_window',
  date: '2026-09-05',
  windows: [
    { jobRef: 'SAKS-1041', customer: 'Priya Patel', window: '09:00 to 11:00', type: 'COLLECTION', confirmed: true },
    { jobRef: 'SAKS-1050', customer: 'Hannah Brooks', window: '13:30 to 15:00', type: 'RETURN', confirmed: true }
  ],
  note: 'Both windows confirmed with customers. A gap of roughly 2.5 hours is built in between stops for the Paignton to Brixham leg and a return to the workshop.'
};
...
const ROUTE_ALERTS = {
  ...
  alerts: [
    {
      severity: 'LOW',
      description: 'Friday afternoon traffic on the A380 Torquay direction can add 20 to 30 minutes on the Brixham leg. Leave the Paignton stop no later than 12:30 to arrive inside the 13:30 window.',
      action: 'Check live traffic on the van tablet before departing stop 1.'
    },
    {
      severity: 'LOW',
      description: 'SAKS-1050 (Hannah Brooks) is two days overdue for return. Customer has been informed. Treat this stop as a priority and do not move the window without checking with the office first.',
      ref: 'SAKS-1050'
    }
  ],
  routePatternNote: 'Thursday routes have averaged overruns in recent months. This is a Friday route but the Brixham leg has similar distance risk. See OPP-002 in the improvement register.'
};
...
const RECORD_OWNERSHIP = [
  ...
  { domain: 'todays_routes', source: '07L Collection & Delivery Route', owner: 'mike_evans', decisionOwner: 'mike_evans' },
  { domain: 'route_customer_contact', source: '07L Route Customer Contacts', owner: 'mike_evans', decisionOwner: 'mike_evans' },
  { domain: 'job_ref_route', source: '07L Route Job References', owner: 'mike_evans', decisionOwner: 'mike_evans' },
  { domain: 'booked_window', source: '07L Customer Booking Windows', owner: 'mike_evans', decisionOwner: 'mike_evans' },
  { domain: 'route_alerts', source: '07L Route Alerts', owner: 'mike_evans', decisionOwner: 'mike_evans' },
...
module.exports = {
  ...
  ASSET_AUTHORISATION,
  TODAYS_ROUTE,
  ROUTE_CUSTOMER_CONTACTS,
  ROUTE_JOB_REFS,
  ROUTE_BOOKED_WINDOWS,
  ROUTE_ALERTS,
  ...
  // The banking FIGURES that used to be re-exported here are gone,
  // deliberately (01/09/2026). They were authored on 01/09 as illustrative
  // detail for the Banking tab, and they contradicted the controlled 07A
  // record they sat beside: a bank balance of GBP 24,680.50 against 07A's
  // GBP 41,800, a VAT reserve of GBP 18,240 against 07A's GBP 9,400, and a
  // VAT quarter that disagreed with the one in TAX_POSITION. Two of those
  // records were in the brain at once, so which figure a worker quoted
  // depended on which record the context builder happened to hand it.
  //
  // The company's money now lives in a real ledger seeded FROM 07A (see
  // lib/scott/finance/), and the live position is supplied to the brain by
  // lib/scott/finance/state.js. One fact, one place. The controls below
  // stay: they are the rules the demonstration operates by, not figures,
  // so nothing can contradict them.
  BANKING_CONTROLS: require('./banking').BANKING_CONTROLS,
  // Lead Finder (15/09/2026). Re-exported rather than duplicated, exactly
  // as the social and banking records are, so the candidates are filtered
  // by clearance.filterAndRedact on the same rule as every other record
  // and reach the Company Brain, search and a worker's context with
  // nothing new to keep in step. Domain commercial_prospecting, which only
  // the owner persona holds.
  LEAD_FINDER: require('./leadFinder').OPPORTUNITIES,
  RECORD_OWNERSHIP
};
