...
async function runEvolutionBriefing({ force = false, now = new Date() } = {}) {
  try {
    ...
    const [added, rejected, retracted, pendingCount, faults] = await Promise.all([
    ...
    // read.
    if (!force && added.length === 0 && rejected.length === 0 && retracted.length === 0 && escalations.length === 0) {
    ...
    if (!result.sent) {
      ...
      return { sent: false, reason: result.reason, added: added.length, rejected: rejected.length, escalations: escalations.length };
    }

    await repo.addActivity({
      actor: 'system',
      eventType: 'brain_digest_sent',
      summary: `Evolution briefing sent to ${result.to}: ${added.length} learned, ${rejected.length} rejected, ${retracted.length} retracted, ${escalations.length} needing a person.`
    });
    return { sent: true, to: result.to, added: added.length, rejected: rejected.length, retracted: retracted.length, escalations: escalations.length, stats: digest.stats };
  } catch (err) {
