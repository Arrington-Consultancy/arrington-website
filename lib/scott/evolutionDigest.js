...
function reasonOf(row) {
  return String(row.decision_note || '').replace(/^(Rejected automatically|Admitted automatically|Not added, already held|Retracted by [^:]*|Corrected by [^:]*|Superseded by #[0-9]+, corrected by [^:]*):\s*/i, '').trim();
}
...
// A rejection that says the company could not reconcile something, as
// opposed to one that refused a fact for its provenance or its size. The
// two markers are the exact wording settleCandidate writes for its conflict
// and consistency branches, so this cannot describe a rule that did not
// run. Tom asked for the serious contradictions specifically, so they lead
// the section and the ordinary refusals follow.
const MAX_GAPS_LISTED = 25;

const CONTRADICTION_MARKERS = [/disagrees with what the company already holds/i, /does not fit the company's own figures/i];

function isContradiction(row) {
  const note = String(row.decision_note || '');
  return CONTRADICTION_MARKERS.some((re) => re.test(note));
}

function buildDigest({ added = [], rejected = [], retracted = [], superseded = [], gapsClosed = [], escalations = [], canon = [], since = null, now = new Date() } = {}) {
  ...
  const lines = [];
  lines.push(`Scott's Armchair & Knitting Service learned ${added.length} new thing${added.length === 1 ? '' : 's'} ${window}, changed ${superseded.length} it already held, and rejected ${rejected.length} it could not take.`);
  lines.push('');

  if (!added.length && !rejected.length && !retracted.length && !superseded.length && !gapsClosed.length) {
  ...
  if (superseded.length) {
    lines.push(`WHAT CHANGED IN THE COMPANY'S STATE (${superseded.length})`);
    lines.push('  Facts the company used to hold and now holds differently. Nothing was overwritten: the old version is still on the register at /scott/gaps with what it said and who changed it.');
    superseded.forEach((r) => {
      lines.push(`    - ${r.domain}/${r.fact_key}`);
      lines.push(`      was: ${r.previous_value}`);
      lines.push(`      now: ${r.fact_value}`);
      const why = reasonOf(r);
      if (why) lines.push(`      changed because: ${why}`);
    });
    lines.push('');
  }

  if (gapsClosed.length) {
    lines.push(`GAPS CLOSED BY LOGIC (${gapsClosed.length})`);
    lines.push('  Nothing here needed a person. A gap closes when something the company learned answers it, when the answer turned out to be on file already, when it blocked nothing, or when it went unfilled long enough to be stale. A hole that still matters is raised again the next time a worker hits it.');
    // A backlog clear can close dozens at once, and a briefing nobody
    // finishes reading is a briefing nobody reads. The count above is the
    // real one; the list is capped and says so rather than trailing off.
    const shown = gapsClosed.slice(0, MAX_GAPS_LISTED);
    shown.forEach((g) => {
      lines.push(`    - ${g.expected_source || g.domain || 'a record'}: ${g.missing_evidence}`);
      if (g.resolution_note) lines.push(`      ${g.resolution_note}`);
    });
    if (gapsClosed.length > shown.length) {
      lines.push(`    and ${gapsClosed.length - shown.length} more, all on the register at /scott/gaps.`);
    }
    lines.push('');
  }
  ...
  if (rejected.length) {
    const contradictions = rejected.filter(isContradiction);
    const others = rejected.filter((r) => !isContradiction(r));
    lines.push(`REJECTED, NOT IN THE COMPANY BRAIN (${rejected.length})`);
    if (contradictions.length) {
      lines.push(`  ${contradictions.length} could not be reconciled with what the company already holds or with its own figures. These are the ones worth reading: each is a moment the demonstration caught itself about to say two different things.`);
    ...
    if (others.length) {
      lines.push(`  ${others.length} refused for other reasons (no source named, the wrong size for this company, nothing stated). Listed so the refusals are visible; nothing to do.`);
      others.forEach((r) => {
        lines.push(`    - ${r.domain}/${r.fact_key}: ${r.fact_value}`);
        const why = reasonOf(r);
        if (why) lines.push(`      because: ${why}`);
  ...
  return {
    subject: `Scott's Armchair & Knitting: ${added.length} learned, ${superseded.length} changed, ${rejected.length} rejected${escalations.length ? ', needs you' : ''}`,
    text: lines.join('\n'),
    stats: {
      added: added.length,
      estimates: estimates.length,
      rejected: rejected.length,
      contradictions: rejected.filter(isContradiction).length,
