...
function openingJournal({ finance, director, tax, borrowing, equipment }, yearJournals) {
  ...
  const termLoan = toPence((borrowing.find((b) => b.kind === 'term_loan') || {}).outstandingPrincipalGbp || 0);
  const equipFinance = toPence((borrowing.find((b) => b.kind === 'equipment_finance') || {}).outstandingGbp || 0);
  ...
  const credits = [
    ['2300', termLoan],
    ['2310', equipFinance],
    ['2400', toPence(director.directorsLoanAccount.currentBalanceGbp)],
    ['3000', toPence(100)]
  ];
