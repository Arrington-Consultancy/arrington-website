...
async function sendLoginNotification({ username, learnedRecently = null, when = new Date() } = {}) {
  ...
  // Since 13/09/2026 nothing waits for approval: the company settles its
  // own additions and a person oversees them afterwards. The alert says
  // how much it has learned lately and where to read it.
  const waiting = Number.isInteger(learnedRecently)
    ? (learnedRecently === 0
      ? 'The company has learned nothing new in the last seven days.'
      : `The company has learned ${learnedRecently} new thing${learnedRecently === 1 ? '' : 's'} in the last seven days. Read them, and retract anything that reads wrong, at ${SITE}/scott/gaps.`)
    : 'What the company has learned lately could not be counted just now.';
  try {
    await transport.sendMail({
      from: NOTIFY_FROM,
      to,
...
module.exports = {
  ...
  digestIsDue,
  sendEvolutionDigest,
  // Exposed for the one-shot production mail proof only, so the check
  // exercises the SAME transport object the login alert and the evolution
  // briefing use rather than a lookalike built beside it. Named so that is
  // obvious at the call site; nothing in the running app reads it.
  __transportForProof: getTransport,
