...
const RESERVED_TOPIC_PATTERNS = [
  ...
  // contract/agreement/arrangement co-occurring with a commercial noun:
  // an unsigned or undated one is still a commercial commitment the
  // document reserves, and the reviewer's probes named "agreement" and
  // "arrangement" without ever using "signed". A bare word match (no
  // commercial-noun requirement) was tried first and caught the reviewer's
  // probes, but governance review 2 (finding N1) found it also refused
  // ordinary staff-rota/workflow questions ("what arrangement do we have
  // for who opens up on Mondays") that have nothing to do with a supplier
  // or customer commitment. The commercial-noun requirement closes that
  // false-positive without reopening the original gap — both of the
  // reviewer's probes name a supplier/merchant/terms/payment/order
  // alongside the contract word.
  { label: 'signed_contracts', pattern: /\b(contract|agreement|arrangement)\b.*\b(supplier|customer|merchant|client|account|terms|payment|order|price|cost|charge|deal)\b|\b(supplier|customer|merchant|client|account|terms|payment|order|price|cost|charge|deal)\b.*\b(contract|agreement|arrangement)\b|\bnda\b|non.?disclosure/i },
...
async function findRelevantFacts(personaId, workerId, message, limit = 6) {
  ...
  // workerId convention). This function itself now fails closed: a falsy
  // personaId sees nothing, tested directly for '', null and undefined.
  //
  // Governance review 2 (31/08/2026), finding N2: correcting a claim
  // this comment previously made, rather than the code. It is NOT true
  // that "an unset persona now sees nothing" for every real caller. The
  // one production path with a genuinely unset persona — the anonymous
  // public lead enquiry's fire-and-forget AI draft
  // (routes/scott.js -> runScottTurnAndPersist with no personaId key) —
  // never reaches this function with a falsy value at all, because
  // contextBuilders.js's buildContext() resolves a falsy personaId to
  // clearance.DEFAULT_PERSONA (the owner) ONE LEVEL UP, before ever
  // calling formatMemoryFactsBlock/this function — the exact same
  // pre-existing fallback every other piece of deep-brain context data
  // already receives on that identical path (see buildContext's own
  // "preserves exact v0.1 behaviour" comment). So that one anonymous
  // context in practice retrieves existing memory facts at owner-level
  // visibility, bounded by the routed worker's own domain permission —
  // not nothing. This is not a new departure from existing behaviour,
  // and no logged-in human with narrower clearance is shown anything
  // their clearance denies: this fallback only ever runs for the
  // anonymous drafting path, whose output already requires a human's
  // approval downstream before it reaches anyone. Left as documented,
  // deliberate, pre-existing behaviour rather than redesigned here — see
  // CLAUDE.md's "Do not redesign unrelated Scott features" instruction.
  if (!personaId) return [];
