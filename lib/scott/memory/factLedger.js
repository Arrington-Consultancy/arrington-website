...
const RESERVED_TOPIC_PATTERNS = [
  ...
  { label: 'signed_contracts', pattern: /\b(signed (a |the )?(contract|agreement)|\bnda\b|non.?disclosure)\b/i },
  ...
  { label: 'external_platform_activity', pattern: /\b(followers|engagement rate|impressions|click.?through|actual (facebook|instagram|linkedin|x) (post|activity))\b/i },
  { label: 'quality_inspection_result', pattern: /\b(passed inspection|failed inspection|quality (sign.?off|audit) result)\b/i },
  { label: 'consequential_customer_promise', pattern: /\b(guarantee|promise(d)?|refund|compensat(e|ion)|discount of|free of charge)\b/i },
  { label: 'predictive_future', pattern: /\b(will be|next (year|quarter|month)|forecast|predict(ion)?|going to (be|happen))\b/i }
];
...
function classifyReasonableness({ workerId, domain, canonicalQuestion }) {
...
async function findRelevantFacts(personaId, workerId, message, limit = 6) {
  ...
  const scored = rows
    .filter((row) => clearance.isDomainVisible(personaId || clearance.DEFAULT_PERSONA, workerId, row.domain))
    .map((row) => {
...
async function establishFact({ workerId, domain, canonicalQuestion, answerText, askedByPersonaId }) {
  const gate = classifyReasonableness({ workerId, domain, canonicalQuestion });
  if (!gate.allowed) {
