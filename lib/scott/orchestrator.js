...
// NOT YET WIRED IN — imported ahead of the next implementation step, which
// is to call extractEntities(userMessage) once per turn in runTurn() below,
// then pass the resulting entities into buildContext(workerId, ...) inside
// callWorker/callReceptionist and splice the result into buildUserContent.
// See the Scott implementation handoff record in Drive for the exact next
// action. Left imported-but-unused deliberately rather than reverted, so
// the next session sees exactly where this stopped instead of re-deriving
// it from scratch.
...
async function callWorker(workerId, { userMessage, history, priorWorkerNotes, routeReason }) {
...
async function runTurn({ userMessage, history }) {
  ...
  return { receptionist, workerReplies };
}
