...
function extractJson(text) {
  ...
  try {
    ...
    // The text that failed to parse travels with the error, so the retry
    // in callWithRetry can tell a reply that stopped mid-object from one
    // that is genuinely malformed. Without this the caller has only the
    // parser's message, which says where it gave up and nothing about
    // whether the object was ever finished.
    err.rawText = candidate;
    throw err;
  }
}
...
- Say so in your "reply" ONCE, in a short clause, as an estimate and not as a filed figure, then move on. "I have not got that on file, but for a business this size it is about X" is the shape and the whole of it.

  SAY IT ONCE. The interface already prints a "LIKELY, NOT CERTAIN" badge under your answer from the "certainty" field, so the reader is told twice before you write a second word about it. A reply that opens by saying it cannot give certified figures, qualifies the number in the middle, and closes by saying it should be treated as a working figure rather than relied on has said the same thing four times and sounds like it does not believe itself. Lead with the figure, caveat it in a clause, stop. Do not write "I'd treat that as a working figure rather than something you can rely on", do not explain a second time at the end which record you are missing, and never apologise for the estimate you were asked to make.
...
- Estimate ORDINARY MANAGEMENT FIGURES freely, whether they are totals or belong to one named person. The monthly wage bill, how much holiday the team has left, how much holiday a particular person has left, someone's contracted hours, the going rate for their role, the headcount cost of a pay rise: a real HR or finance system would hold every one of these, so estimate them from the headcount, the role, the trade and the company's own numbers, and say it is an estimate. These people are fictional and have no privacy to protect, so "I will not estimate that for a named person" is the wrong answer here.
- What you do NOT invent is an adverse or intimate characterisation of a named person: that someone is ill, in a grievance or a disciplinary, underperforming, or has difficulties at home. Not because of privacy, but because making something unflattering up about a named colleague adds nothing and reads badly. If the record says it, report it. If it does not, do not author it. Counting how many people are in some category is a management figure and is fine; deciding WHICH named person is in an adverse one is not.
- Where an aggregate is genuinely made of parts you cannot see, build it anyway from what you can: how many people there are, what the business turns over, what roles they do, and what such roles are paid in this trade. Say that is how you built it.
...
// THE OPERATING SNAPSHOT REACHES EVERY LEVEL. This is deliberate and it
// reverses what this file did earlier the same day (15/09/2026).
//
// The first build of the progression replaced CURRENT_OPERATING_POSITION
// with a "you cannot see this yet" block at Level 1, because Level 1 also
// capped the retrieved context to one source class and the live probe
// caught Ruth answering a capacity question from the prompt rather than
// the context. The fix was sound for the model it served.
//
// Tom then settled the model, and it is a different one: SAME BRAIN, MORE
// TOOLS, BIGGER WORKSPACE. A level says how much of the workspace is on
// screen. It does not say what a person is allowed to know, and it must
// never be able to: moving Scott from Level 4 to Level 1 must leave his
// authorised knowledge exactly where it was, and moving Mike Evans from
// Level 1 to Level 4 must not hand him a single thing his clearance does
// not already grant. Both directions are asserted by test.
//
// So there is no level-dependent prompt any more. What survives from that
// work is the part that was never about levels: the standing rule in
// GOVERNANCE_PREAMBLE that a worker must not invent a figure it cannot
// derive from something it can actually see. That rule protects against
// the failure the withheld block was really guarding — an invented number
// presented as a fact — and it protects at every level, for every persona,
// which is where the protection belonged all along.
// TODAY'S DATE, and why it is a function rather than a constant.
//
// Nothing told the workers what day it was, so asked directly they said so,
// and Ruth answered a visitor "I can't tell you today's real date". A
// business assistant that does not know the date is not a small gap, it is
// the first thing that makes it look like a toy.
//
// Computed per call, NEVER at module load. GOVERNANCE_PREAMBLE is a
// module-level constant, so a date interpolated there would be frozen at
// the moment the container booted and would drift a day further from the
// truth every day the deployment stayed up, which is a worse failure than
// having no date at all: wrong and confident rather than absent.
function todayBlock(now = new Date()) {
  return `TODAY
Today is ${now.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}. Use it for anything that depends on the date, and answer plainly if somebody simply asks what day it is.`;
}
...
${worker.personality}`;
  ...
  return [GOVERNANCE_PREAMBLE, todayBlock(), BRAND_AND_OPERATING_SYSTEM, CURRENT_OPERATING_POSITION, identityBlock, OUTPUT_FORMAT_WORKER].join('\n\n');
}
...
If the request contains more than one owned component, list each in dependency order. Do not invent a new worker. Do not answer specialist questions yourself. Your job is routing, plus a short note.`;
  ...
  return [GOVERNANCE_PREAMBLE, todayBlock(), BRAND_AND_OPERATING_SYSTEM, CURRENT_OPERATING_POSITION, identityBlock, OUTPUT_FORMAT_RECEPTIONIST].join('\n\n');
}
...
// True when text is JSON that stopped in the middle rather than JSON that
// is merely wrong: more opening braces or brackets than closing ones, or
// an odd number of unescaped quotes. Deliberately structural rather than a
// message match, because the parser's wording is not ours to rely on.
// Returns false for anything it cannot see, so an unknown case retries
...
function looksUnterminated(text) {
  ...
  for (let i = 0; i < text.length; i += 1) {
    const c = text[i];
    if (c === '\\') { i += 1; continue; }
    if (c === '"') { inString = !inString; quotes += 1; continue; }
  ...
  return braces > 0 || brackets > 0 || inString || quotes % 2 === 1;
}
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      if (!isTransientApiError(err)) {
        // A parse failure that is really a truncation in disguise.
        //
        // Found live on 01/09/2026: the receptionist was cut off at 512,
        // correctly retried at 1024, and the second reply came back as
        // "Unexpected token in JSON at position 1045" and landed here,
        // which breaks immediately. So a reply that ran out of room twice
        // was reported to the visitor as a technical problem, when the
        // only thing wrong was that we had not left space for it.
        //
        // The model does not always mark the second one as a stop-reason
        // truncation, so the shape of the text is the evidence: JSON that
        // fails to parse AND has more opening braces than closing ones is
        // an object that never finished. Retried with more room, once,
        // and only while there is room left to give.
        if (looksUnterminated(err.rawText) && ceiling < MAX_TOKEN_CEILING && attempt < 3) {
          console.error('Scott model call: parse failure looks like a cut-off reply, retrying with more room');
          lastTransient = err;
          ceiling = Math.min(ceiling * 2, MAX_TOKEN_CEILING);
          brevityNote = '\n\nKeep the reply short and complete: two or three sentences of plain text in the JSON, and close the JSON object properly. A cut-off reply is worse than a brief one.';
...
// WORKSPACE NOTE, a deterministic fact about the workspace rather than a
// fact about the business.
//
// It says "this workspace has no tool for that" and it is decided in code
// (lib/scott/capabilityRegistry.js), never by the model. It is carried in
// the USER content rather than the system prompt on purpose: it is true of
// this turn, not of this worker, and a system prompt that changed per turn
// would be an invitation to start varying other things there too.
//
// It never grants, never widens, and never mentions a record. The worst a
// malformed one can do is make a worker say it cannot carry an action out.
function formatWorkspaceNote(note) {
  return note ? `WHAT THIS WORKSPACE CAN DO RIGHT NOW\n${note}` : '';
}

async function callReceptionist({ userMessage, history, workspaceNote }) {
  const systemPrompt = buildReceptionistSystemPrompt();
  const historyBlock = formatHistoryForPrompt(history);
  const buildUserContent = () => [
    historyBlock,
    formatWorkspaceNote(workspaceNote),
    `NEW MESSAGE FROM THE USER\n${userMessage}`,
    'Reply with the JSON object only.'
  ].filter(Boolean).join('\n\n');

  // Raised from 512 on 01/09/2026. Ruth routes across nine workers, and a
  // broad question ("where can we make improvements in the business?")
  // legitimately names several of them with a reason each. At 512 those
  // answers were being cut off, and the retry then produced a reply that
  // failed to parse, so the visitor saw a technical problem where the only
  // fault was the room we had left her. Her replies are still short by
  // instruction; this is headroom, not a licence to ramble.
  const result = await callWithRetry(systemPrompt, buildUserContent, validateReceptionistReply, 1024);
  if (!result.ok) {
    return {
...
async function callWorker(workerId, { userMessage, history, priorWorkerNotes, routeReason, entities, personaId, workspaceNote }) {
...
async function runTurn({ userMessage, history, personaId, workspaceNote }) {
  ...
  const receptionist = await callReceptionist({ userMessage, history, workspaceNote });
