...
async function callWorker(workerId, { userMessage, history, priorWorkerNotes, routeReason, entities, personaId }) {
  ...
  if (data.memoryFact) {
    ...
    if (outcome.ok) {
      data.memoryFact = { ...data.memoryFact, established: true, wasNewlyCreated: outcome.created, factId: outcome.fact.id };
      if (!outcome.created && outcome.fact.answer !== data.memoryFact.answer) {
        // Lost a simultaneous first-write race: the reply this visitor
        // already received cannot be silently rewritten, but the STORED
        // fact — which is what every later question retrieves — is the
        // one canonical row, never the loser's text. Recorded here so a
        // future turn (and the memory context block) sees only the
        // canonical value.
        data.memoryFact.canonicalAnswer = outcome.fact.answer;
        data.memoryFact.conflictResolved = true;
      }
    } else {
