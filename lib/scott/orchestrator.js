...
// THE LEVEL 1 PROMPT, and why this block exists.
//
// CURRENT_OPERATING_POSITION is the operating snapshot: weekly capacity,
// stock counts, lead times and the price list. It is joined into EVERY
// worker's and Ruth's system prompt, which means it reaches them outside
// buildContext and therefore outside both the clearance filter and the
// progression's source-class cap.
//
// That made the Level 1 ceiling untrue, and the live probe caught it: asked
// about next week's capacity at Level 1, Ruth answered "this week we're at
// 10 of 12 repair slots booked". The retrieved context held nothing but
// leads, exactly as designed — the figure came from the prompt.
//
// So at Level 1 the snapshot is replaced by a line saying it is not
// available yet. The permanent brand and commercial rules stay, because
// they are rules and tone rather than position, and a worker without them
// would answer out of character and outside its own guardrails.
//
// This is a presentation-level narrowing, like the rest of the
// progression: it removes something at Level 1 and adds nothing anywhere.
// Above Level 1 the prompt is byte-for-byte what it was.
const LEVEL_1_POSITION_WITHHELD = `CURRENT OPERATING SNAPSHOT: not available at this step.

You do not have the operating snapshot. That means you do not know this
week's workshop or knitting capacity, current stock counts, lead times, or
the price list, and you must not state or estimate any of them. Say plainly
that you cannot see them yet. Do not reconstruct them from memory, from the
business description above, or from what is typical for a business like
this.`;

function positionBlockFor(level) {
  return level === 1 ? LEVEL_1_POSITION_WITHHELD : CURRENT_OPERATING_POSITION;
}

function buildWorkerSystemPrompt(worker, level) {
...
${worker.personality}`;
  ...
  return [GOVERNANCE_PREAMBLE, BRAND_AND_OPERATING_SYSTEM, positionBlockFor(level), identityBlock, OUTPUT_FORMAT_WORKER].join('\n\n');
}
...
function buildReceptionistSystemPrompt(level) {
...
If the request contains more than one owned component, list each in dependency order. Do not invent a new worker. Do not answer specialist questions yourself. Your job is routing, plus a short note.`;
  ...
  return [GOVERNANCE_PREAMBLE, BRAND_AND_OPERATING_SYSTEM, positionBlockFor(level), identityBlock, OUTPUT_FORMAT_RECEPTIONIST].join('\n\n');
}
...
async function callReceptionist({ userMessage, history, level }) {
  const systemPrompt = buildReceptionistSystemPrompt(level);
...
async function callWorker(workerId, { userMessage, history, priorWorkerNotes, routeReason, entities, personaId, level }) {
  const worker = getWorker(workerId);
  const systemPrompt = buildWorkerSystemPrompt(worker, level);
  const historyBlock = formatHistoryForPrompt(history);
  ...
  const dataContextBlock = await buildContext(workerId, { message: userMessage, entities: entities || {}, personaId, level });
  const buildUserContent = () => [
    historyBlock,
    priorNotesBlock,
    dataContextBlock,
...
async function runTurn({ userMessage, history, personaId, level }) {
  ...
  const receptionist = await callReceptionist({ userMessage, history, level });
