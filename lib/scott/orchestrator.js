...
const memory = require('./memory/factLedger');
...
function validateWorkerReply(obj) {
  ...
  // change): a worker proposes a NEW fact only when neither the
  // controlled evidence nor an existing memory fact (both already shown
  // to it in its own context block) answered the question, and the topic
  // is genuinely ordinary and low-consequence. This is validated for
  // shape only — the actual reasonableness/clearance/domain gate lives in
  // lib/scott/memory/factLedger.js and runs in callWorker below,
  // independently of whether the model followed its own instructions.
  if (obj.memoryFact !== null && obj.memoryFact !== undefined) {
    if (typeof obj.memoryFact !== 'object' || Array.isArray(obj.memoryFact)) {
      errors.push('memoryFact must be an object or null');
    } else {
      if (!isNonEmptyString(obj.memoryFact.domain)) errors.push('memoryFact.domain must be a non-empty string');
      if (!isNonEmptyString(obj.memoryFact.canonicalQuestion)) errors.push('memoryFact.canonicalQuestion must be a non-empty string');
      if (!isNonEmptyString(obj.memoryFact.answer)) errors.push('memoryFact.answer must be a non-empty string');
    }
  }
...
const OUTPUT_FORMAT_WORKER = `Respond with ONLY a single JSON object, no prose before or after it, no markdown code fence, matching exactly this shape:
{
  ...
  "memoryFact": { "domain": string, "canonicalQuestion": string, "answer": string } | null,  // only if you are establishing a BRAND NEW ordinary, low-consequence fictional business fact (see EVOLVING FICTIONAL MEMORY above). Never set this for a fact already shown to you as PREVIOUSLY ESTABLISHED FICTIONAL COMPANY MEMORY, and never for anything reserved/consequential/unknowable (use "gap" or a plain "we don't hold that" reply instead).
...
"memoryFact.domain" must be one of the small set of ordinary operating domains this demonstration allows new fictional memory in (marketing_performance, suppliers_ops, materials) and must actually be a domain your own worker permission covers. Do not guess a domain you do not hold.`;
...
async function callWorker(workerId, { userMessage, history, priorWorkerNotes, routeReason, entities, personaId }) {
  ...
  // Evolving fictional business memory (31/08/2026 approved design
  // change): the reasonableness/clearance gate is enforced here, in code,
  // independently of whatever the prompt told the model. A model that
  // proposes a fact outside the allowed domains, outside its own worker
  // permission, or matching a reserved topic never reaches the database —
  // classifyReasonableness() (lib/scott/memory/factLedger.js) refuses it
  // and the reply is corrected to an honest "not held" answer rather than
  // silently keeping the model's own invented text, which would otherwise
  // let a prompt failure become a persisted (if merely unpersisted-but-
  // stated) fabrication anyway.
  if (data.memoryFact) {
    const outcome = await memory.establishFact({
      workerId,
      domain: data.memoryFact.domain,
      canonicalQuestion: data.memoryFact.canonicalQuestion,
      answerText: data.memoryFact.answer,
      askedByPersonaId: personaId || ''
    });
    if (outcome.ok) {
      if (!outcome.created) {
        // Governance review 1 (31/08/2026), finding M3: "must not
        // generate a different answer merely because the model is called
        // again" was enforced for what got STORED, but not for what got
        // SAID — the model's own reply text for this turn was left
        // untouched, so a model that ignored the instruction to reuse a
        // previously-established fact verbatim could still SPEAK a
        // drifted answer even though the ledger correctly kept the
        // original. This covers both the ordinary same-question-again
        // case and a lost simultaneous first-write race identically:
        // either way, what is stored is canonical and what is said must
        // match it, unconditionally, not only when a difference happens
        // to be detected.
        data.reply = outcome.fact.answer;
        data.certainty = 'CERTAIN';
      }
      data.memoryFact = { ...data.memoryFact, established: true, wasNewlyCreated: outcome.created, factId: outcome.fact.id, answer: outcome.fact.answer };
    } else {
      data.memoryFact = { established: false, refusedReason: outcome.reason };
      data.reply = `That's not something the records here would reasonably hold, so I can't confirm it rather than guess.`;
      data.certainty = 'UNPROVEN';
    }
  }
  return { workerId, worker, ...data, technicalFailure: false };
}
