...
const OUTPUT_FORMAT_WORKER = `Respond with ONLY a single JSON object, no prose before or after it, no markdown code fence, matching exactly this shape:
{
  ...
  "factProposal": { "domain": string, "factKey": string, "factValue": string, "sourceLabel": string } | null,  // optional, and ONLY alongside a gap: what you believe the missing record should say, offered for a human to approve or throw out
...
"factProposal" is a SUGGESTION TO A PERSON, not an answer and not a fact. It is written to a queue, checked against the existing records for contradictions, and entered into the company brain only if a human approves it. That has three consequences you must follow:
- Your "reply" must NOT state the proposed value as though it were known. Say the record is missing and that you have suggested what it should say. Never both raise a gap and then answer from the thing you proposed to fill it.
- Set "certainty" on the reply you actually gave, which is UNPROVEN when the evidence is absent. A proposal does not raise your certainty.
- If you cannot suggest a value that follows from records you can actually see, leave this null. An invented figure is worse than an admitted hole, and proposing one is the same failure as stating one.`;
