...
const WORKERS = {
  ...
  // QUALITY CONTROL] - PROPOSED WORKER SPECIFICATION and the personality
  // lock in 23 SCOTT V0.2 FULL COMPLETION BRIEF. Doc 24 SCOTT V0.2
  // INDEPENDENT GOVERNANCE & ASSURANCE REVIEW REQUEST carries the status
  // "FORMALLY PREPARED FOR INDEPENDENT REVIEW - NO VERDICT RECORDED" as of
  // this transcription (29/08/2026) — no PASS, no AMBER, no STOP. Per the
  // brief's own instruction, that settles it without needing to ask: these
  // three stay `active: false` until a later session finds an actual
  // independent verdict plus Tom's explicit activation record in Drive.
  //
  // active: false has two enforced effects, both load-bearing:
  //   1. ACTIVE_WORKER_IDS / ROUTABLE_WORKER_IDS below exclude them, so
  //      Ruth's receptionist routing (orchestrator.js) can never send a
  //      live user turn to one of these three — there is no code path
  //      that reaches buildWorkerSystemPrompt() for an inactive worker.
  //   2. The portal (routes/scott.js, workers.js consumers) must render
  //      them as "proposed, not yet active" wherever staff are listed —
  //      never silently presented as live team members. See
  //      test/scott/workers.test.js for the assertion that both hold.
  finance_accounts: {
    id: 'finance_accounts',
    active: false,
    ...
    accent: '#2E6B4F',
    initials: 'NP',
    tagline: 'Proposed, not yet active. Management accounts, cash, debtors/creditors, affordability.',
    purpose: 'Own fictional management accounts, cash visibility, debtor/creditor control, affordability checks, budget monitoring and financial interpretation for Scott\'s Armchair & Knitting Service, once activated.',
    scope: [
      ...
      'Flag where accountant, payroll, tax or legal evidence would be required rather than inventing an answer'
    ],
    ...
    permissionsSummary: 'May read approved Scott authorities, 07A Finance & Accounts, relevant customer/trade records, current operating evidence and its own handoff once activated. May calculate, analyse and propose financial actions. No external payment, bank action, debt collection send, tax filing or financial commitment.',
  ...
  people_hr: {
    ...
    accent: '#7A5C8E',
    initials: 'SK',
    tagline: 'Proposed, not yet active. Absence, training, fair process, working patterns.',
    purpose: 'Own fictional people records, absence and leave interpretation, training status, recruitment process, working-pattern questions and fair HR-process guidance for Scott\'s Armchair & Knitting Service, once activated.',
    scope: [
    ...
    permissionsSummary: 'May read approved Scott authorities, 07B People & HR, current operating evidence necessary for staffing context and its own handoff once activated. May calculate staffing implications, draft internal HR process notes and propose people actions. No external employment communication, contract issue, offer, dismissal or payroll change.',
  ...
  quality_control: {
    ...
    accent: '#B3541E',
    initials: 'NH',
    tagline: 'Proposed, not yet active. Quality holds, defects, rework, release readiness.',
    purpose: 'Interpret recorded inspection evidence, control quality holds, classify defects and rework, identify repeat failures, and protect the rule that defective work is not released merely to rescue a promised date, once activated.',
    scope: [
