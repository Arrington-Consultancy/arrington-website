...
const ARTICLES = [
  ...
  {
    // Added per Tom's request, supplied directly (not sourced from a
    // handover doc) — connected into Commercial Gaps Review rather than
    // the Owner Dependency Quiz, tagged "demand" (real question: "Is
    // demand for the business currently something you have to chase, or
    // something that mostly comes to you?"). The article's own central
    // contrast (chasing an introduction and deciding whether to use the
    // name, versus the unprompted call where "the phone rang") is a
    // precise, direct match for that question, more so than any other
    // category in the real taxonomy. Reassigned outright from
    // case_study_aesthetic_clinics_margin in
    // lib/commercialGapsResources.js (which keeps its other genuine
    // category, "pricing"), same approach used for "margin" and
    // "capacity" earlier, so only one resource claims "demand".
  ...
  {
    // Added 09/08/2026 following a full Drive reconciliation/inventory
    // pass — "The Monument to Wasted Money" is marked "Finished and saved
    // as a standalone Useful Thinking article" in the Arrington Useful
    // Thinking Map (Drive), supplied directly by Tom, not sourced from a
    // handover doc. Tagged "commercial_priorities" (real question:
    // "Right now, what matters most to you commercially: growing revenue,
    // protecting margin, freeing up your own time, or something else?")
    // — the article's own closing question ("What impact will it have on
    // your core business? And how distracted are you going to become
    // from your bread and butter?") is a precise, direct match. Reassigned
    // outright from case_study_abacus_falmouth in
    // lib/commercialGapsResources.js (which keeps its other two genuine
    // categories, "owner_dependency" and "business_model"), same approach
    // used for "margin", "capacity" and "demand" earlier, so only one
    // resource claims "commercial_priorities". No related-link: the Map
    // confirms no explicit companion-piece relationship was set for this
    // article, unlike Serendipity/Some People or the two-way link
    // instruction that exists for those.
    slug: 'the-monument-to-wasted-money',
    instanceId: 'article__12',
    published: true,
    cgrCategory: 'commercial_priorities'
  },
  {
    // Added 09/08/2026, supplied directly by Tom via the current Google
    // Drive document ("Useful Thinking - You Build a Business One Problem
    // at a Time"), reproduced verbatim. Deliberately left unmapped to both
    // recommendation systems per Tom's explicit instruction: it is the
    // first Useful Thinking piece built around the sale/exit of one of
    // Tom's businesses, and its real subject (an owner only seeing the
    // whole shape of what they built when a buyer's outside perspective
    // forces them to explain it) does not match the Owner Dependency
    // Quiz's dependency-risk categories or any category in the Commercial
    // Gaps Review's fixed taxonomy without forcing the fit. It sits ready
    // for a future sale-readiness / business-value recommendation surface
    // that does not exist yet, rather than being shoehorned into either
    // existing one.
  ...
  {
    // Added 26/08/2026, supplied directly by Tom via the current Google
    // Drive document ("Useful Thinking - You Can Train, But You Shouldn't
    // Blame"), reproduced verbatim from the public-facing portion only
    // (see db/seed.js for the exact exclusion of the Drive doc's internal
    // notes). Deliberately left unmapped to both recommendation systems:
    // the Drive doc's own internal notes name a buyer-language problem
    // ("Why does everyone still come to me for decisions?") that would
    // naturally map to the Commercial Gaps Review's decision_making
    // category, but that category is already claimed by "Being Certain
    // Isn't the Same as Being Right" (article, the first published piece)
    // — reassigning an existing category mapping wasn't part of the
    // publish-this-article instruction, so it's flagged for Tom rather
    // than decided here, same as the reassignment pattern used elsewhere
    // in this file for margin/capacity/demand/commercial_priorities.
    slug: 'you-can-train-but-you-shouldnt-blame',
    instanceId: 'article__14',
    published: true
  },
  {
    // Added 03/09/2026 from the controlled Google Drive document
    // ("Useful Thinking - The Most Expensive Person Doing the Job Might
    // Be You", Drive id 1k5JFUiIkASvOy6cpCwIhbCMZnt6cyAL1HldVhWsjKNk),
    // which carries Tom's own status line: "TOM-APPROVED USEFUL THINKING
    // ARTICLE. Ready for Website & Hosting worker publication."
    // Reproduced verbatim from the public-facing portion only; the
    // Drive doc's "INTERNAL NOTES - NOT FOR PUBLICATION" block is
    // excluded, the same exclusion applied to every previous article.
    // The named 4Com reference, the "slightly shit hand" line and the
    // closing "wrong fucking job" line are Tom's approved wording and
    // are preserved exactly, not softened.
    //
    // Left unmapped to the Commercial Gaps Review, following the
    // article__14 precedent immediately above. The Drive doc's internal
    // notes name a buyer-language problem ("Why do the businesses that
    // most need to cut costs have the least time to go and find them?")
    // which would plausibly map to missed_opportunities, still free in
    // the taxonomy. But the instruction for this piece was to publish it
    // through the existing workflow, not to decide a recommendation
    // mapping, and the controlling handover could not be read (see the
    // seed migration's note). Flagged for Tom rather than decided here.
    //
    // NOT to be confused with the held article__5, "The Reverse Economy
    // of Scale". That is a separate, still-unpublished piece on a
    // related growth theme; this article does not supersede or release
    // it.
    slug: 'the-most-expensive-person-doing-the-job-might-be-you',
    instanceId: 'article__15',
    published: true
  },
  {
    // Added 04/09/2026 from the controlled Google Drive document
    // ("Useful Thinking - The Little Things You Have to Give Up", Drive
    // id 1TKK6daYMUhHkxttollqY_o-Ewya9OYPzzakJlHqoaas), which carries
    // Tom's own status line: "TOM-APPROVED USEFUL THINKING ARTICLE.
    // Ready for Website & Hosting worker publication."
    //
    // The Drive document carries an explicit PERSONAL DISCLOSURE
    // CONTROL: the nicotine and younger Texas Hold'em references are
    // approved personal disclosure, confirmed by Tom, and must not be
    // removed, softened or generalised. The same protection was stated
    // for "don't get caught", "come out in the wash", the observations
    // about good managers being lost, and the closing line. All are
    // reproduced exactly. See db/seed.js for the full note.
    //
    // RECORD GAP, second instance: the publication instruction cited a
    // newly created Website & Hosting handover for this article, and no
    // such document exists in Drive. Searched by title, by full text,
    // by creation date on 04/09/2026, and inside the Website & Hosting
    // Worker Handoff Log. The same happened the previous day with
    // "Website Worker Handover 05" (see the record-gap document in
    // Drive). The article carries its own approval and the workflow was
    // copied from the fourteen before it, so nothing was invented, but
    // this is now a pattern rather than a one-off and is reported as
    // such.
    //
    // Left unmapped to the Commercial Gaps Review, following the two
    // articles before it. The Drive notes name a buyer-language problem
    // ("I don't know how to step back") which maps naturally to
    // owner_dependency, but that category is already claimed by "The
    // Customer Who Messaged Tom at 4am", and reassigning an existing
    // mapping was not part of the instruction. Flagged for Tom rather
    // than decided here, the same pattern used for article__14 and
    // article__15.
    slug: 'the-little-things-you-have-to-give-up',
    instanceId: 'article__16',
    published: true
  }
];
