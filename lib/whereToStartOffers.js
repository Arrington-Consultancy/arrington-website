...
const INTEGRATION_IDENTIFIER_PREFIX = 'arrington_wts';

// publicPriceApproved — whether a figure may be QUOTED IN A NEW SURFACE.
//
// This is not the price. `pricePence` is, and it is what Stripe charges and
// what the Where to Start pages display; none of that is changed by this flag
// and none of it should be read from it.
//
// CURRENT STATE (14/09/2026, second decision): all five are approved, so the
// Product Guide quotes the same figures as the rest of the live site.
//
// It was briefly narrower on the same day. Tom's first decision approved £999
// only, withholding £500 as an unapproved published Commercial Review price
// and £2,500/£3,400 as conflicting figures. Seeing the list rendered, he
// asked the right question: why are only two priced? The answer that settled
// it is that all three withheld figures were ALREADY public one click away —
// /where-to-start lists all five, and each offer page carries its own price
// and Pay button — so withholding them on the guide protected nothing and
// only made the list read as though three prices had gone missing.
//
// The reconciliation of £500, £2,500 and £3,400 remains open as its own
// controlled work item. Approving them for display here is not approving the
// figures themselves; it is declining to have one page be quieter than every
// other page about numbers the site already publishes.
//
// The mechanism is kept rather than deleted, because it is the place a future
// withheld price gets recorded, and because a price is never invented to fill
// a gap: an absent figure is an honest statement, a substitute would not be.
...
const OFFERS = {
  conversation: {
    ...
    pricePence: 0,
    // Free, and "no charge" is a fact rather than a price claim.
    publicPriceApproved: true,
    purchasable: false
  },
  commercial_review: {
    id: 'commercial_review',
    name: 'Commercial Review',
    path: '/where-to-start/commercial-review',
    ...
    pricePence: 50000,
    currency: 'gbp',
    // Approved for display 14/09/2026 (second decision); see the note above.
    publicPriceApproved: true,
    purchasable: true,
  ...
  full_commercial_review: {
    ...
    name: 'Commercial Review and Implementation',
    path: '/where-to-start/full-commercial-review',
    // Deliberately not framed as the serious look versus the £500 review,
    // that implies the £500 Commercial Review is not also serious standalone
    // work, which it is. The real distinction is depth, access and what
    // happens afterwards: practical implementation help.
    description: 'A deeper commercial review with implementation support: more access, evidence followed rather than assumed, and practical help putting the right things in place once the direction is agreed.',
    pricePence: 250000,
    currency: 'gbp',
    // Displayed 14/09/2026; the figure's own reconciliation stays open.
    publicPriceApproved: true,
  ...
  website_build: {
    id: 'website_build',
    name: 'Website Build',
    path: '/where-to-start/website-build',
    description: 'A fixed-price website build. A recorded planning conversation to agree what it needs to do, a complete first version, and one structured round of changes before launch.',
    pricePence: 99900,
    currency: 'gbp',
    // Tom, 14/09/2026: approved, and may be shown.
    publicPriceApproved: true,
    purchasable: true
  },
  full_review_website_build: {
    id: 'full_review_website_build',
    name: 'Commercial Review and Website Build',
    path: '/where-to-start/full-review-website-build',
    description: 'A commercial review followed by a website build shaped by what the business needs.',
    pricePence: 340000,
    currency: 'gbp',
    // Displayed 14/09/2026; the figure's own reconciliation stays open.
    publicPriceApproved: true,
...
function buildCheckoutSessionParams({ offer, email, successUrl, cancelUrl, chargeAmountPence, creditAppliedPence }) {
  ...
  const params = {
    ...
    integration_identifier: `${INTEGRATION_IDENTIFIER_PREFIX}_${randomLetters(8)}`,
    metadata: {
      offer_id: offer.id,
