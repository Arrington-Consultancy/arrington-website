...
// what the existing Where to Start pages display; none of that is changed by
// this flag and none of it should be read from it.
//
// It exists because Tom's decision of 14/09/2026 separated two things that
// had been one: what the checkout charges, and what the business is willing
// to publish as a standing price on a new page. £999 is approved for both.
// £500 is explicitly not approved as a published Commercial Review price,
// and £2,500 and £3,400 are recorded as conflicting figures awaiting an
// approval-evidence reconciliation that is its own controlled work item.
//
// So a new surface (the Product Guide) reads THIS, and quotes nothing for an
// offer that is false here. Removing the existing live exposure of £500,
// £2,500 and £3,400 on the Where to Start pages is deliberately NOT what
// this flag does: that is the reconciliation item, and doing it as a side
// effect here would be exactly the unapproved pricing change the flag is
// meant to prevent.
//
// A price is never invented to fill the gap. The absent figure is the
// honest statement; a substitute would not be.
...
const OFFERS = {
  ...
  commercial_review: {
    ...
    pricePence: 50000,
    currency: 'gbp',
    // Tom, 14/09/2026: not approved as a published Commercial Review price.
    publicPriceApproved: false,
    purchasable: true,
  ...
  full_commercial_review: {
    ...
    pricePence: 250000,
    currency: 'gbp',
    // Conflicting figure awaiting the reconciliation work item.
    publicPriceApproved: false,
