...
// Case-INSENSITIVE, and the `i` is load-bearing.
//
// Governance finding R1 (31/08/2026), HIGH: this regex was case
// sensitive and Express's routing is not. server.js never sets `case
// sensitive routing`, so Express's default applies and `/API/workspace/ask`
// reaches the same handler - while this guard, matching only lower case,
// let it straight past to the automatic OPTIONS responder. One capital
// letter reopened the whole Q1 enumeration oracle, anonymously, with the
// enable flag off.
//
// The rule to carry: a guard that decides on a path must match paths the
// same way the router does, or it is guarding a different application.
