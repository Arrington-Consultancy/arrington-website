...
// Both maps below are NULL-PROTOTYPE, and finding V5 is why.
//
// They are keyed by values that arrive from configuration (a username
// from WORKSPACE_OWNER_USERNAME, a clearance id derived from it), and a
// plain object literal answers `constructor`, `__proto__`, `toString`
// and the rest from Object.prototype. The truthiness guards below then
// pass, because a function is truthy, and the next line throws instead
// of refusing. Nothing reaches this today - clearanceId is only ever
// clearanceForUser(user), and no request-derived value is used - but
// describeOwnerBinding() shares the lookup, so a WORKSPACE_OWNER_USERNAME
// of `toString` would print the binding as ok for a username that holds
// no clearance in code. A boot diagnostic reporting a gate as configured
// when it is not is exactly what finding G7 was.
//
// This is the same one-line fix already applied in lanes.js at T3.
const CLEARANCES = Object.assign(Object.create(null), {
  owner_admin: {
    label: 'Owner / Admin',
...
const HUMAN_CLEARANCE = Object.assign(Object.create(null), { tom: 'owner_admin' });
