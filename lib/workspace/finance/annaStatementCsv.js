...
function parseStatementCsv(csvText) {
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    if (col.balance !== -1) {
      const b = parseAmountToPence(r[col.balance]);
      if (b !== null) { lastBalancePence = b; lastBalanceDate = date; }
    }
  }

  transactions.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
  // If the CSV carried a balance column, the LATEST dated row's balance
  // is the statement's closing balance, not necessarily the last row as
  // printed (a statement is not guaranteed to be in date order).
  if (col.balance !== -1 && transactions.length > 0) {
    const latestDate = transactions[transactions.length - 1].date;
    if (lastBalanceDate !== latestDate) {
      // Recompute against the actually-latest row rather than trusting
      // iteration order.
      for (let i = 1; i < rows.length; i += 1) {
        const r = rows[i];
        if (parseDate(r[col.date]) === latestDate) {
          const b = col.balance !== -1 ? parseAmountToPence(r[col.balance]) : null;
          if (b !== null) lastBalancePence = b;
        }
      }
    }
  }
  ...
  return {
    ...
    warnings,
    columnsFound: col,
    closingBalancePence: col.balance !== -1 ? lastBalancePence : null,
    closingBalanceDate: col.balance !== -1 ? (transactions.length ? transactions[transactions.length - 1].date : null) : null
  };
}

module.exports = { parseStatementCsv, parseCsvLines, parseDate, parseAmountToPence, contentHash };
