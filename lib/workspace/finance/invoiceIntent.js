...
const SEND_VERB_RE = /\b(send|email)\b[^.?!]{0,40}\binvoice\b|\binvoice\b[^.?!]{0,30}\b(and|then) (send|email)\b|\b(send|email) it\b/i;
...
function parse(message, { today = new Date() } = {}) {
  ...
  if (!f.text || !INTENT_RE.test(f.text)) return { matched: false };

  const missing = [];
  if (!f.email) missing.push('the customer email address');
  if (f.amount == null) missing.push('the amount (for example £500)');
  if (!f.description) missing.push('what it is for (for example "for commercial review")');
...
module.exports = { parse, describe, extractFields, modeFromSentence, EMAIL_RE, INTENT_RE, DRAFT_ONLY_RE, SEND_VERB_RE };
