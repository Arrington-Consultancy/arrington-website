...
// "invoice Tim Hunt at World Student Advisors £533" (15/09/2026). The
// word "invoice" is the VERB here, there is no "to" or "for" within
// twenty characters of it and no address anywhere in the sentence, so
// neither INTENT_RE nor the loose reading below fired. The sentence went
// to the model instead, which offered to "have the invoice prepared for
// your approval" and then described an invoicing queue acting on Tom's
// instruction. Neither was true: no draft was raised, because this
// parser is the only thing that raises one.
//
// So a sentence that uses one of these words as a verb AND carries an
// amount is an invoice request, with or without an address. It is still
// only a DRAFT behind an approval card, so a wrong reading costs one
// "discard"; the failure it replaces cost a real client invoice.
const VERB_AMOUNT_RE = /\b(invoice|bill|charge)\b/i;
...
function extractFields(message, { today = new Date() } = {}) {
  ...
  if (email) {
    ...
    // No address in the sentence, so the customer is whoever sits
    // between the verb and the amount: "invoice TIM HUNT AT WORLD
    // STUDENT ADVISORS £533". Without this the draft card reads "to (no
    // customer)" and loses the one part of the request Tom did type.
    const verb = text.match(/\b(?:invoice|bill|charge)\b\s+/i);
    if (verb) {
      let span = text.slice(verb.index + verb[0].length);
      span = span.replace(/^(?:for|to)\s+/i, '');
      const amt = span.match(AMOUNT_RE);
      if (amt) span = span.slice(0, amt.index);
      span = span.replace(/\b(for|to|at|dated|as of|on|due)\s*$/i, '').trim().replace(/["'<>()]/g, '').replace(/[\s,;:.]+$/, '');
      // An article or a pronoun on its own is not a customer, and a long
      // run of words is a sentence rather than a name.
      if (span && !/^(a|an|the|it|this|that|them|him|her|us|me)$/i.test(span) && span.split(/\s+/).length <= 8) customerName = span;
    }
  }
  ...
  if (forIdx >= 0) {
    ...
    // names the customer after the first "for" and the amount after the
    // second, so the tail here is the customer with a preposition left
    // hanging off it. A dangling preposition is the tell, and the job
    // description is the one field nobody should have to correct on a
    // real invoice.
    tail = tail.replace(/\b(for|to|at|of|with)\s*$/i, '');
  ...
  // The customer is not the job. Where both readings land on the same
  // words the sentence named a customer and never said what the invoice
  // is for, so the draft asks rather than inventing an answer.
  if (description && customerName && description.toLowerCase() === customerName.toLowerCase()) description = '';
...
// Asking about invoices is not instructing one. "How much did we invoice
// Orca, £533?" must never raise a draft. A sentence that OPENS with a
// request word is an instruction however it ends, because "can you
// invoice Tim Hunt £533?" is a request with a question mark on it, not a
// question about the business.
const REQUEST_OPENER_RE = /^\s*(please|can|could|would|will|let'?s|i (need|want|would like)|we (need|want))\b/i;
const QUESTION_OPENER_RE = /^\s*(what|what'?s|which|who|whose|whom|when|where|why|how|did|do|does|has|have|had|is|are|was|were|should|shall)\b/i;
function isQuestion(message) {
  const text = String(message || '').trim();
  if (!text) return false;
  if (REQUEST_OPENER_RE.test(text)) return false;
  return QUESTION_OPENER_RE.test(text) || /\?\s*$/.test(text);
}
...
function parse(message, { today = new Date() } = {}) {
  ...
  if (isQuestion(f.text)) return { matched: false };
  const loose = !!f.email && f.amount != null && LOOSE_VERB_RE.test(f.text);
  // The verb-with-amount reading (15/09/2026): no address needed, so the
  // draft lands incomplete and asks for one, rather than the whole
  // request falling through to the model.
  const verbAmount = f.amount != null && VERB_AMOUNT_RE.test(f.text);
  if (!INTENT_RE.test(f.text) && !loose && !verbAmount) return { matched: false };
...
module.exports = { parse, describe, extractFields, missingFor, modeFromSentence, isQuestion, EMAIL_RE, INTENT_RE, DRAFT_ONLY_RE, SEND_VERB_RE };
