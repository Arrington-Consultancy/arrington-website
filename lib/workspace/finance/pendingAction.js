...
function invoiceChanges(question, pending, today) {
  ...
  if (f.email && f.email.toLowerCase() !== String(draft.customerEmail || '').toLowerCase()) {
    ...
    // An address typed on its own to complete a draft carries no name,
    // so extractFields falls back to the local part ("tim.hunt"). That
    // must not replace a customer the owner actually typed ("Tim Hunt at
    // World Student Advisors") in the sentence that raised the draft.
    // Only where the address is FILLING a blank one, though: swapping one
    // address for another ("change the email to bob@example.com") is a
    // different customer, and the old name must not follow it across.
    const localPart = f.email.split('@')[0];
    const supplied = f.customerName && f.customerName !== localPart ? f.customerName : '';
    const fillingBlank = !String(draft.customerEmail || '').trim();
    if (supplied) changes.customerName = supplied;
    else if (!(fillingBlank && draft.customerName)) changes.customerName = localPart;
  ...
  // An incomplete draft is waiting for exactly this kind of sentence
  // ("for a test"), so no change verb is needed to fill a gap.
  if (description && (pending.incomplete || intent.INTENT_RE.test(question) || CHANGE_VERB_RE.test(question)) && description !== draft.description) changes.description = description;
...
function resolve(question, pending, { today = new Date() } = {}) {
  ...
  if (isInvoice) {
    ...
    if (hasChanges && (pending.incomplete || REFERS_RE.test(text) || intent.INTENT_RE.test(text) || changes.date)) {
    ...
    // An incomplete draft missing only its description is waiting for a
    // short phrase: "a test", "website build". No question, no field,
    // no other instruction, and short enough to be a job name.
    if (pending.incomplete && !SEND_NOW_RE.test(text) && !/\?\s*$/.test(text) && text.length <= 60) {
      const f = intent.extractFields(text, { today });
      const missing = intent.missingFor(pending.draft || {});
      if (missing.length === 1 && /what it is for/.test(missing[0]) && !f.email && f.amount == null) {
        const description = text.replace(/^(it'?s|it is|that'?s|this is)\s+(for\s+)?/i, '').replace(/^for\s+/i, '').replace(TRAILING_FILLER_RE, '').replace(/[.!]+$/, '').trim();
        if (description && !SHOW_RE.test(text)) return { matched: true, op: 'amend', changes: { description: description.charAt(0).toUpperCase() + description.slice(1) } };
      }
    }
...
function describePending(pending) {
  ...
  if (pending.kind === 'zoho_invoice_draft') {
    ...
    const missing = intent.missingFor(pending.draft || {});
    if (missing.length) {
      return `Pending: approval #${pending.id}, INCOMPLETE invoice ${intent.describe(pending.draft)}. Still needs ${missing.join(', and ')}. Mode: ${modeText}. It cannot be approved until it is complete; nothing has been created.`;
    }
