...
// design and from lib/workspace/social/registry.js, and apply to ALL
// providers:
//
...
const PROVIDERS = {
  ...
  zoho_invoice: {
    id: 'zoho_invoice',
    name: 'Zoho Invoice (EU)',
    primary: false,
    api: 'Zoho Invoice API v3 (EU data centre, invoice.zoho.eu), read-only',
    authRoute: 'Zoho OAuth 2.0 authorization code flow; refresh token stored in ZOHO_INVOICE_REFRESH_TOKEN (set once by Tom after the one-time consent dance)',
    credentialEnv: ['ZOHO_INVOICE_CLIENT_ID', 'ZOHO_INVOICE_CLIENT_SECRET', 'ZOHO_INVOICE_REFRESH_TOKEN'],
    readScopes: ['ZohoInvoice.invoices.READ', 'ZohoInvoice.customerpayments.READ', 'ZohoInvoice.contacts.READ'],
    // Writes (06/09/2026, Tom's instruction): create a customer, create
    // a draft invoice, email it. CREATE only, never UPDATE/DELETE/
    // fullaccess, requested at consent only while
    // ENABLE_ZOHO_INVOICE_WRITES is 'true', and human-initiated only.
    // Declared separately from readScopes, like social's mutationScopes,
    // so the read-only test on readScopes keeps guarding exactly what it
    // always guarded. An invoice asks a customer for money; it moves
    // none, so MONEY_ACTION_CLASS_NEVER_BUILT is unaffected.
    writeScopes: ['ZohoInvoice.contacts.CREATE', 'ZohoInvoice.invoices.CREATE'],
    writesFlag: 'ENABLE_ZOHO_INVOICE_WRITES',
    capabilities: AUTONOMOUS_CAPABILITIES,
    supports: {
      balance: false,  // Zoho Invoice is an invoicing tool, not a bank account
      transactions: false,
      categories: false,
      recurring: false,
      payees: true,    // contacts (customers)
      invoices: true,
      receipts: false  // customer payments, not receipts per se
    },
    setupNote: 'Requires ZOHO_INVOICE_CLIENT_ID and ZOHO_INVOICE_CLIENT_SECRET in Railway. Visit /workspace/finance/zoho/connect while logged in, authorise in Zoho, then copy the refresh token that appears once and set ZOHO_INVOICE_REFRESH_TOKEN in Railway. Redeploy to activate. Organisation ID 20119226503 is hardcoded in the client.'
  }
};
