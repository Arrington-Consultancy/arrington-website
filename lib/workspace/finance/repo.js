...
// response.
async function recordCsvImport(providerId, { transactions, warnings = [], closingBalancePence = null, closingBalanceDate = null, importedBy }) {
...
async function syncFinanceSummaryRecord(now = new Date()) {
  ...
  const lines = [
    ...
    recent.length ? `Most recent ${recent.length} transaction(s):` : 'No transactions have been synced or imported yet.'
  ];
  recent.forEach((t) => {
