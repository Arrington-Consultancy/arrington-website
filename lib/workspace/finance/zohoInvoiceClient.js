...
const API_BASE = 'https://invoice.zoho.eu/api/v3';
...
function buildAuthorizeUrl(state) {
  const params = new URLSearchParams({
    response_type: 'code',
    client_id: process.env.ZOHO_INVOICE_CLIENT_ID || '',
...
async function _tokenPost(body) {
  const clientId = process.env.ZOHO_INVOICE_CLIENT_ID || '';
  const clientSecret = process.env.ZOHO_INVOICE_CLIENT_SECRET || '';
...
async function getAccessToken() {
  ...
  _refreshPromise = (async () => {
    const refreshToken = process.env.ZOHO_INVOICE_REFRESH_TOKEN || '';
