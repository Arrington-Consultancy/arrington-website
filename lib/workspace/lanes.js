...
const SOURCE_CLASSES = {
  ...
  // Added 01/09/2026, Tom's ANNA MONEY BANKING INTEGRATION DECISION.
  // Read-only business banking: balance, transactions, categories,
  // recurring payments where available, provenance/freshness. Deliberately
  // granted to NO lane below (least privilege: no canonical worker's
  // approved remit currently includes finance data), so only
  // governance_assurance can read it via a lane, and Tom's own general
  // questions via GENERAL_SOURCE_CLASSES in orchestrator.js. Widening this
  // to a lane is a worker-permission change and belongs to Tom plus the
  // governed route, same as every other source class here.
  finance: 'Read-only business banking: balance, transactions, categories, recurring payments where available, provenance/freshness',
  // Added 07/09/2026 on Tom's instruction ("a Gmail api we can use
  // directly in Arrington Workspace"). One snapshot record of Tom's own
  // inbox: unread count plus headers and one-line previews of the most
  // recent messages, never bodies. Same least-privilege position as
  // finance: granted to NO lane, so only governance_assurance reads it
  // via a lane and Tom's own general questions via GENERAL_SOURCE_CLASSES.
  email: 'Inbox snapshot from the Gmail API: unread count, recent senders, subjects and one-line previews; no bodies',
  // Added 15/09/2026 on Tom's instruction, after an invoice went out at a
  // figure that omitted a logged row. One record: the hours log's entries,
  // billable flags, rate and totals by client. Same least-privilege
  // position as finance and email, and for a sharper reason than either:
  // this record carries Arrington's own charging rate and the notes
  // saying which entries are estimates rather than stopwatch time, so a
  // worker lane holding it is a commercial disclosure and not merely a
  // wider read. Granted to NO lane. Only governance_assurance reaches it
  // via a lane, and Tom's own general questions via GENERAL_SOURCE_CLASSES
  // in orchestrator.js. Widening it to a lane is a worker-permission
  // change and belongs to Tom plus the governed route.
  hours: 'Hours log: dated work entries per client, billable flags, the hourly rate and totals; read-only from one pinned Google Sheet'
};
...
// Null-prototype, deliberately. Governance finding T3 (31/08/2026):
