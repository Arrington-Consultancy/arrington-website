...
// Where the low-precedence tail begins. Everything before it is a head
// rule that predates this change; everything from it on is an inflection
// repair that may only claim a question no head rule wanted.
const TAIL_STARTS_AT = 9;
...
const ROUTING_RULES = [
  ...
  // with a blind slice, and listRecords orders by source_class, so the
  // alphabetically-last classes go first. With a full snapshot, "which
  // clearances exist?" lost worker_register entirely, which the general
  // context keeps. Repairing an inflection is not worth a truncated
...
module.exports = {
  ...
  __routingTableForTests: Object.freeze(ROUTING_RULES.map((r, i) => Object.freeze({
    ...
    // Which half of the table this rule is in, derived from the constant
    // below rather than left for a test to hardcode: a test that assumes
    // the boundary would classify a newly added head rule as the first
    // tail rule and quietly stop checking it.
    tail: i >= TAIL_STARTS_AT
  }))),
