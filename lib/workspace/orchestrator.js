...
const ROUTING_RULES = [
  ...
  //
  // ORDER WITHIN THE TAIL is by lane breadth, narrowest first, measured
  // as the number of source classes the lane reads (lanes.js). A tail
  // rule can only ever claim a question no rule above wanted, so the
  // ordering decides nothing except which lane answers a question naming
  // two tail subjects, and there the narrower lane should win: task
  // necessity is a permission leg, so the wider lane winning by accident
  // of register order would widen it for no reason.
  //
  // An earlier version ordered this group by register order instead,
  // which contradicted that sentence: "draft social posts about our
  // campaigns" reached google_ads rather than the narrower
  // social_content_builder. A test now derives the expected order from
  // lanes.js rather than restating it here, so the two cannot drift.
