...
const ROUTING_RULES = [
  ...
  //
  // Without this, the opportunity rule below captures "where are the
  // opportunities to reduce our recurring costs?" and the banking records
  // vanish from a cost question, because lanes.js grants 'finance' to
  // governance_assurance alone and the general context is its only other
  // route. The fix deliberately does NOT hand finance to a lane: that
  // would be a worker-permission change and is not this change's to make.
  // It restores the routing such a question had before the opportunity
  // rule existed, and it widens nothing, because the general context is
  // still filtered by the human clearance leg and finance records are
  // 'confidential'.
  //
  // Placed below every specific lane on purpose: "what does governance
  // say about our costs?" still reaches governance_assurance, which holds
  // finance anyway.
  { laneId: null, pattern: /\b(cash ?flow|bank(ing|s)?|balances?|transactions?|invoices?|payments?|overheads?|costs?|spend(ing)?|turnover|profits?|margins?|vat|payroll|recurring (cost|payment)s?)\b/i },
...
module.exports = {
  MODEL,
  LANES,
  // Exported for tests only: the routing tests assert structural
  // properties of the rule list and of the general context, and
  // example-based checks alone were shown to pass while the property
  // they were named for was false.
  ROUTING_RULES,
