...
const ROUTING_RULES = [
  ...
  //
  // ORDER WITHIN THE TAIL is deliberate, not incidental: the
  // commercial-ceiling lanes come first in their original order, then the
  // two confidential-ceiling lanes, narrowest first. A tail rule can only
  // ever claim a question no rule above wanted, so this ordering decides
  // nothing except which lane answers a question that names two tail
  // subjects, and there the narrower lane should win.
