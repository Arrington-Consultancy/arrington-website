...
const ROUTING_RULES = [
  ...
  // above wanted, so it can take nothing from any lane.
  //
  // What it CAN do, and the comment must not pretend otherwise, is move a
  // question from the general no-lane context into a lane. For the two
  // confidential-ceiling lanes that is a wider context: "show me the
  // audits" goes from a four-class general context to
  // governance_assurance, which reads all eight. No lane gained a
  // permission and the human clearance leg still gates every record
  // returned, which a test asserts with a positive control, but task
  // necessity is a permission leg and this widens it. It extends the
  // behaviour the singular forms have always had rather than inventing
  // it, and governance_assurance is the correct owner of the word
  // "audits", so it is recorded as a bounded consequence rather than
  // claimed to be no change. The
  ...
  // by vocabulary: ANY question that routes to a lane loses finance,
  // unless that lane is governance_assurance. That is a property of
  // finance being granted to one lane, not of this change, and the
  ...
  // What this change does add is more questions that route at all, so it
  // makes an existing trade more visible; the trade itself is not new.
...
async function askWorkspace({ clearanceId, question, laneId: forcedLaneId = null }) {
  ...
  const system = [
    GOVERNANCE_RULES,
    lane ? `This question was routed to the ${lane.name} lane. Its remit: ${lane.remit} Answer within that remit.` : 'This question matched no specialist lane. Answer only from the records supplied below.',
    REPLY_CONTRACT
  ].join('\n\n');
  const userContent = [
...
module.exports = {
  ...
  __routingTableForTests: Object.freeze(ROUTING_RULES.map((r) => Object.freeze({
