...
// nicety.
const ROUTING_RULES = [
  { laneId: 'google_ads', pattern: /\b(google ads|paid (ads|advertising|media)|ppc|adwords|campaigns?|cost per (lead|click)|conversion tracking)\b/i },
  { laneId: 'website_hosting', pattern: /\b(websites?|hosting|deploys?|railway|github|domains?|dns|cms|servers?|stripe|checkouts?|seo tags?)\b/i },
  ...
  { laneId: 'brain_keeper', pattern: /\b(drive|brain (index|structure|maintenance)|document status(?:es)?|superseded|archives?|handoff standards?)\b/i },
  ...
  { laneId: 'social_content_builder', pattern: /\b(linkedin|social (content|posts?|media)|story banks?|published posts?)\b/i },
  { laneId: 'ai_recommendation_visibility', pattern: /\b(ai (visibility|recommendation)|cited by ai|chatgpt recommend|shortlists?)\b/i },
  { laneId: 'ai_demonstration_builder', pattern: /\b(scott|demonstrations?|armchairs?|knitting|fictional)\b/i },
  ...
  // "opportunity" / "opportunities", deliberately in the tail below and
  // deliberately not in the opportunity rule above.
  //
  ...
  // gained both confidential opportunity records, measured against the
  // routing test's own fixture. Human clearance still gated the
  // result, but task necessity is a permission leg in its own right and
  // that widened it. Placed in the tail the word is what it actually is,
  // a weak signal: a question that names a later lane's subject still
  // goes to that lane, and only a question about opportunities and
  // nothing else lands here. (Not "any" lane: the third and fifth rules
  // still win first, so "which Drive document holds the proposal?" goes
  // to opportunity_builder, as the next paragraph says.)
  //
  ...
  // Tom's to take.
  // ---- Low-precedence tail. Everything below runs only when no rule
  // ---- above matched, so nothing here can take a question away from a
  // ---- lane that already wins it.

  // The two confidential-ceiling lanes get their missed plurals HERE
  // rather than in their own rules, and the asymmetry is deliberate.
  //
  // Repairing them in place would extend those lanes' existing
  // pre-emption of the later commercial-ceiling lanes to new phrasings:
  // "draft a LinkedIn post about our prospects" would leave
  // social_content_builder for a confidential-ceiling lane. Fixing a
  // routing miss by pushing questions into a confidential lane is exactly
  // what this work is not allowed to do, so the plurals sit where they
  // can only ever claim a question no other lane wanted.
  //
  // The singular forms keep their existing high precedence, untouched, in
  // rules three and five. That leaves singular and plural on different
  // precedence, which is a real inconsistency and is the safe direction:
  // resolving it means moving live behaviour, which is its own decision.
  //
  // Governance comes before the money rule below, and that ordering is
  // the whole point of the rule that follows: governance_assurance is the
  // one lane granted 'finance', so a money question landing there loses
  // nothing. "Show me the audits of our spending" belongs in that lane.
  ...
  //
  // The principle, stated once so the ordering above and below is not
  // arbitrary: DO NOT ROUTE A MONEY QUESTION INTO A LANE THAT CANNOT SEE
  // FINANCE. lanes.js grants 'finance' to governance_assurance alone, so
  // that lane is placed above this rule and the opportunity tail below
  // it. Without this, the opportunity rule captured "where are the
  // opportunities to reduce our recurring costs?" and the banking records
  // vanished from a cost question.
  //
  // Deliberately a tail-only device. Rules one to nine are untouched, so
  // "what did we spend on Google Ads?" still reaches google_ads exactly
  // as it did before; changing that is a separate decision about lanes
  // that hold no finance class.
  //
  // The trade this imposes, both halves stated and both tested: a
  // question naming money AND opportunities keeps finance and loses the
  // opportunity records, because the general context has no 'opportunity'
  // source class. Neither context dominates the other. The genuinely
  ...
  //
  // The fix deliberately does NOT hand finance to a lane. It widens
  // nothing: the general context is still filtered by the human clearance
  // leg and finance records are 'confidential'.
  { laneId: null, pattern: /\b(cash ?flow|bank(ing|s)?|balances?|transactions?|invoices?|payments?|overheads?|costs?|spend(ing)?|turnover|profits?|margins?|vat|payroll)\b/i },
...
async function askWorkspace({ clearanceId, question, laneId: forcedLaneId = null }) {
  ...
  const system = [
    GOVERNANCE_RULES,
    lane ? `This question was routed to the ${lane.name} lane. Its remit: ${lane.remit} Answer within that remit.` : 'This question matched no specialist lane. Answer only from the records supplied below, which are the core authority and register state plus any finance records the reader is cleared for.',
    REPLY_CONTRACT
  ].join('\n\n');
  const userContent = [
