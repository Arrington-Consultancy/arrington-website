...
const TAIL_RULES = [
  ...
  //
  // NARROWED TO THE ONE APPROVED WORD, which is the lesson of this whole
  // change rather than a detail. Earlier versions also repaired
  // 'prospects', 'proposals' and 'commercial conversations' here. Because
  // this is the only confidential-ceiling rule in the tail, any question
  // whose own vocabulary was NOT repaired falls past every commercial
  // rule and lands on it: "what proposals has the brain keeper archived?"
  // reached no lane at the base and reached this lane, gaining both
  // confidential opportunity records and losing control_pack. The
  // narrowest-first ordering cannot help, because it only orders the
  // lanes whose vocabulary exists.
  //
  // Successive review rounds each found another word with that shape:
  // 'pipelines' (infrastructure), 'opportunity cost' (a finance term),
  // 'campaigns' (email and social, not only paid), then 'proposals'
  // arriving from a brain-keeper question. That is not a run of unlucky
  // words. Vocabulary cannot decide which LANE a question belongs to when
  // the lanes differ in sensitivity ceiling, and each repair widens the
  // surface for the next one.
  //
  // So this rule carries exactly the word Tom approved. The
  // commercial-ceiling rules above keep their repairs, because a question
  // landing in the wrong COMMERCIAL lane costs subject matter, not
  // confidentiality. 'prospects', 'proposals' and 'commercial
  // conversations' route as they did at the base, singulars yes and
  // plurals no, recorded as remaining work rather than repaired at this
  // price.
  //
  // 'opportunity cost' is excluded because it is a fixed finance term,
  // not a reference to the pipeline. The lookahead hangs off the SINGULAR
  // branch only, because English separates the senses by number:
  // "opportunity cost" is the noun phrase, "opportunities cost" is a
  // plural noun and a verb. A first attempt put it after the whole group
  // and blocked the verb sense too, so "which opportunities cost us the
  // most to pursue?" stopped routing. [\s-]* covers the hyphen and the
  // double space, and cost(s|ing)?\b covers all three shapes of the term
  // WITHOUT swallowing longer words: an earlier version dropped the
  // trailing boundary to catch 'costing' and thereby blocked "is this
  // opportunity costly to pursue?", "is the opportunity costed yet?" and
  // "is there an opportunity Costa Coffee would fund?", losing every
  // opportunity record for exactly the questions this rule exists to
  // route. "Which opportunity costs the most?" is blocked
  // too, a singular subject with a plural verb rather than the fixed
  // term, and left blocked deliberately: it is a cost question, and the
  // general context is the only place besides governance_assurance that
  // finance can be read.
  { laneId: 'opportunity_builder', pattern: /\bopportunit(?:y(?![\s-]*cost(s|ing)?\b)|ies)\b/i }
];
...
// 'email' (07/09/2026) follows the finance precedent: no lane holds it, so
// the general context is the only way Tom's own inbox snapshot reaches an
// answer. See lanes.js for what the record does and does not contain.
// 'hours' (15/09/2026) follows the same precedent again: no lane holds it,
// so Tom's own general questions are the only routed path to it besides
// governance_assurance, and the human clearance leg still applies (the
// record is 'confidential', which only owner_admin holds).
const GENERAL_SOURCE_CLASSES = Object.freeze(['authority', 'strategy', 'worker_register', 'finance', 'email', 'hours']);
...
async function buildLaneContext({ clearanceId, laneId }) {
  ...
  return selectContextRecords(current, MAX_CONTEXT_RECORDS);
}

// Choose which permitted records reach the prompt when there are more of
// them than the cap.
//
// This does NOT change which records a lane may see. Everything here has
// already passed human clearance and the lane's own source classes and
// ceiling; the only question is which of them fit. Widening is not on
// the table and nothing below can add a record the caller was not
// already entitled to.
//
// It used to be `slice(0, MAX_CONTEXT_RECORDS)` over a list that
// `repo.listRecords` orders by `source_class` ascending, so the records
// that got dropped were whichever ones sorted last, which is a function
// of the alphabet and nothing else. Measured on the real 29-record
// snapshot: governance_assurance reaches 28 against a cap of 24, and the
// four it loses all come off the end of `worker_register`, because "w"
// sorts last. Had a class been named "zzz" it would have been invisible
// to that lane entirely, and nothing would have said so.
//
// One record from each class in turn, repeating, until the cap is
// reached. Every class the lane holds is represented before any class
// takes a second slot, so a thin class is never starved by a fat one and
// no class can be shut out by its initial letter. Records are returned
// in their original order afterwards, so the prompt stays grouped and
// deterministic for the same input.
function selectContextRecords(records, limit) {
  if (records.length <= limit) return records;
  const order = new Map(records.map((r, i) => [r, i]));
  const queues = new Map();
  for (const r of records) {
    if (!queues.has(r.source_class)) queues.set(r.source_class, []);
    queues.get(r.source_class).push(r);
  }
  const picked = [];
  let progressed = true;
...
- If the message names an action awaiting the owner's decision, that is a fact about the queue reported by the server, not something you did. The workspace itself handles changing, cancelling or approving it through the owner's own words; you never state that it has been created, sent, changed or cancelled, and you never supply or guess a customer, an amount or a description for it.
- Never offer to prepare, raise, arrange or queue anything, and never describe another part of the system acting on the owner's instruction. Nothing is waiting unless the line above says so. An invoice is raised only when the owner types the request itself, so where that is what they want, say what to type, for example: invoice Acme Ltd acme@example.com £500 for commercial review.
- Say a caveat ONCE, in one short clause. Where the recent turns show the owner has already been given it and told you to go ahead, do not raise it again; repeating it reads as refusing the instruction. This narrows when a caveat is repeated and never whether it is given: a record marked stale is still described as stale the first time it is used.
- Do not narrate your own limits. One short clause where it matters, never a paragraph about what you are and are not able to do.
...
// The starting token budget, and the most room any one answer may claim.
// Raising the start would spend more on every question; raising the
// ceiling is what gives a long answer somewhere to go.
const START_TOKENS = 1500;
const MAX_TOKEN_CEILING = 4096;

const BREVITY_NOTE = '\n\nKeep the reply short and complete: two or three sentences of plain text in the JSON, and close the JSON object properly. A reply that is cut off is worse than a brief one.';
...
async function callModel(system, userContent, maxTokens) {
  ...
  // No text at all. There is no reply to correct, so asking the model to
  // "reply again with ONLY the JSON object" is answering a question it
  // never asked. Tagged for a plain retry instead, the same reasoning as
  // the cut-off reply below.
  if (!text.trim()) {
  ...
  // A reply cut off at the ceiling is not a badly written reply. It is a
  // correct reply we did not leave room for, and its JSON stops
  // mid-object, so the contract check reports "no JSON object in reply"
  // and the corrective retry asks the model to fix a schema it never got
  // to finish. Retried at the same ceiling it fails the same way, which
  // is exactly what reached Tom on 03/09/2026 as "Model reply broke the
  // contract twice" on a broad question. Detecting it from stop_reason
  // lets the retry do the one thing that helps: give it more room.
...
// The model does not always mark a cut-off reply with stop_reason, so
// the shape of the text is the second piece of evidence: text that fails
// the contract AND has more opening braces or brackets than closing
// ones, or an unclosed string, is an object that never finished rather
// than one written wrongly.
function looksUnterminated(text) {
...
// Where the object starting at `start` closes, or -1 if it never does.
// Braces inside a JSON string do not count, and a backslash inside a
// string escapes the next character, so a reply whose answer text
// contains a quote or a brace is read correctly rather than cut short.
...
//
// This used to be a single greedy /\{[\s\S]*\}/, which is not a JSON
// extractor: it takes everything from the FIRST brace to the LAST one,
// wherever they are. Measured on 03/09/2026, that failed on four shapes
// a model produces routinely, and two of them are likely on exactly the
// commercial questions this workspace is for:
//
//   - prose mentioning a brace before the object ("costs {such as
//     hosting} matter"), which made the extract start in the prose;
//   - any character after the object, down to a ":}" emoticon, which
//     made it end past the object;
//   - the object sent twice, which glued both into one invalid string.
//
// So: strip a markdown fence if the whole reply is one, then walk each
// opening brace and take the first BALANCED region that actually parses.
// Balance alone is not enough, because "{such as hosting}" balances
// perfectly and is not JSON; parsing is what decides.
function extractJsonObject(raw) {
  const text = String(raw == null ? '' : raw);
  if (!text.trim()) return { error: 'no JSON object in reply' };
  ...
  // A balanced region that would not parse is a malformed object, and
  // says so. Nothing brace-shaped at all is the other message, and the
  // difference matters: only the first is worth a schema correction.
  if (lastParseError) return { error: `invalid JSON: ${lastParseError.message}` };
...
function parseReply(raw) {
  ...
  // No shape guard here on purpose. The extractor only ever returns
  // JSON.parse of a balanced {...} region, which is always a plain
  // object or a throw, so a null/array check could never fire. A guard
  // that cannot fire is worse than none: it reads as protection and
  // provides none. The real shape checks are the field checks below.
...
// The bounded conversational context added 07/09/2026, after a follow-up
// about a pending invoice was answered as though it were the first
// message of the day. `history` is the last few turns of this
// conversation (the route caps their number and length); `pendingAction`
// is one server-built sentence describing the approval the conversation
// is waiting on, or null. Both are context, not records: they change no
// leg of the permission model, because the history is the owner's own
// words and the pending line is built from a row the caller has already
// checked the owner's clearance against.
function renderHistoryForPrompt(history) {
  const turns = (Array.isArray(history) ? history : [])
    .filter((m) => m && (m.role === 'user' || m.role === 'assistant') && typeof m.content === 'string' && m.content.trim())
    .slice(-MAX_HISTORY_TURNS)
    .map((m) => `${m.role === 'user' ? 'OWNER' : 'WORKSPACE'}: ${m.content.trim().slice(0, MAX_HISTORY_CHARS)}`);
  return turns.length ? turns.join('\n') : '';
}
const MAX_HISTORY_TURNS = 6;
const MAX_HISTORY_CHARS = 600;

async function askWorkspace({ clearanceId, question, laneId: forcedLaneId = null, history = [], pendingAction = null }) {
  ...
  const historyText = renderHistoryForPrompt(history);
  const pendingText = typeof pendingAction === 'string' && pendingAction.trim() ? pendingAction.trim().slice(0, 600) : '';
  const userContent = [
    historyText
      ? `Recent turns of this conversation, oldest first. Context only, so a follow-up can be understood; they are not records and contain no facts you may use beyond what the records state:\n${historyText}`
      : '',
    pendingText
      ? `Action awaiting the owner's decision (reported by the server): ${pendingText}\nIf the owner's question is about this action, answer from this line. Ordinary follow-ups ("change it to £600", "don't send it", "leave it in drafts", "send it", "show me first", "cancel that") are carried out by the workspace itself when typed in those words, and approving it is done on the card or in Decisions & approvals; if the owner seems to want one of those, say so in those words.`
      : '',
  ...
  let raw = null;
  let lastErr = null;
  // Raised, never reset, so the same question cannot fail the same way
  // three times over. The brevity note rides along with the extra room,
  // because more room alone invites a longer answer.
  let ceiling = START_TOKENS;
  let brevityNote = '';
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      raw = await callModel(system, `${userContent}${brevityNote}`, ceiling);
      break;
    } catch (err) {
      lastErr = err;
      if (err.code === 'EMPTY_REPLY' && attempt < 3) {
        console.error(`Workspace model returned an empty reply (attempt ${attempt} of 3); retrying`);
        await sleep(500 * attempt);
        continue;
      }
      if (err.code === 'TRUNCATED_REPLY' && ceiling < MAX_TOKEN_CEILING && attempt < 3) {
        ceiling = Math.min(ceiling * 2, MAX_TOKEN_CEILING);
        brevityNote = BREVITY_NOTE;
        console.error(`Workspace model reply was cut off; retrying with ${ceiling} tokens`);
        await sleep(500 * attempt);
        continue;
      }
  ...
  // A contract failure that is really a cut-off reply the model did not
  // mark. Asking it to "reply again with ONLY the JSON object" cannot
  // help here, because the object was already only JSON: it just ran out
  // of room. Give it more room instead, once, and only while there is
  // room left to give.
  if (error && looksUnterminated(raw) && ceiling < MAX_TOKEN_CEILING) {
    ceiling = Math.min(ceiling * 2, MAX_TOKEN_CEILING);
    console.error('Workspace model reply looks cut off rather than malformed; retrying with more room');
    try {
      raw = await callModel(system, `${userContent}${BREVITY_NOTE}`, ceiling);
  ...
  if (error) {
    // One corrective retry for a genuine contract failure, then give up
    // honestly.
    try {
      raw = await callModel(system, `${userContent}\n\nYour previous reply broke the required JSON contract (${error}). Reply again with ONLY the JSON object.`, ceiling);
      ({ parsed, error } = parseReply(raw));
    } catch (err) {
      return { ok: false, errors: [`Model unavailable on corrective retry: ${err.message}`] };
    }
...
module.exports = {
  ...
  // Exported for tests only, for the same reason as GENERAL_SOURCE_CLASSES
  // above: the receivables retrieval suite asserts that a record's
  // FRESHNESS reaches the prompt as a word, and testing a copy of this
  // function would prove nothing about the one the model is handed.
