...
// Governance finding X1: this used to filter `Object.keys(opts)`, which
// is OWN enumerable keys only. A field carried on the object's prototype
// was invisible to it while the destructure on the next line read
// straight through the prototype chain, so a prototype-borne
// `recordCount` made her state a count the interface contradicted, with
// no throw. "Anything else THROWS" was therefore not held, in the guard
// whose whole purpose is to hold it.
//
// Not a disclosure - the function reads only the declared names, so a
// prototype-borne `record` was never spoken - and not reachable, because
// the single caller builds an object literal from three computed values.
// Fixed for the reason the reviewer gave and the same one used to delete
// the inert parameter at W1: a mechanism weaker than its comment invites
// a future caller nobody reasoned about.
//
// Both halves are closed. `for...in` sees inherited enumerable keys, so
// an undeclared one throws; and the values are read with Object.hasOwn,
// so a prototype-borne DECLARED name cannot be read either. Either alone
// would leave the other open.
function assertOnlyPermitted(opts) {
  const extra = [];
  for (const k in (opts || {})) {
    if (!NOTE_FIELDS.includes(k)) extra.push(k);
  }
  if (extra.length) {
...
// Read a declared field, and only from the object itself. The guard above
// refuses an undeclared inherited key; this refuses an inherited value
// for a declared one.
function own(opts, field) {
  return Object.hasOwn(opts, field) ? opts[field] : undefined;
}
...
function handoffNote(opts = {}) {
  ...
  // Read through `own`, not by destructuring: see finding X1 above.
  const laneId = own(opts, 'laneId');
  const recordCount = own(opts, 'recordCount');
  const gapRaised = own(opts, 'gapRaised');
