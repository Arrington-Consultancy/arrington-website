...
// Plain text search over title, body and record key. Returns raw rows;
// the caller filters for clearance BEFORE computing any count it shows,
// so result sizes leak nothing.
//
// record_key was added to the search on 16/09/2026. The key
// is printed on screen in two places - the Company Brain's own list and
// the gaps register's "record <key>" line - so a key you can read and
// cannot search for is a dead end, and the Today page's "See the record"
// link is exactly that search. It found nothing before this.
//
// No clearance consequence: the caller filters what comes back through
// filterRecordsForClearance, so this widens what can be FOUND by someone
// already entitled to it, never who may see a record.
async function searchRecords(term) {
  const { rows } = await db.query(
    `SELECT * FROM workspace_records
     WHERE title ILIKE $1 OR body ILIKE $1 OR record_key ILIKE $1
...
// Amend an approval that is still open: a pending action the owner is
// changing before deciding it ("change it to £600", "leave it in
// drafts"). One row is revised in place rather than a second row being
// created, so the queue never holds a superseded copy next to the live
// one. A decided row is never touched: the person read what they
// approved, and that is what stays on the record.
async function updateOpenApproval(id, { title, detail }) {
  const { rows } = await db.query(
    `UPDATE workspace_approvals SET title = $2, detail = $3
     WHERE id = $1 AND status = 'open'
     RETURNING *`,
    [id, title, detail]
  );
