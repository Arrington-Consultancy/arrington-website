...
const PLATFORMS = {
  facebook: {
    ...
    readScopes: ['pages_show_list', 'pages_read_engagement', 'pages_read_user_content', 'read_insights'],
    // Scopes that CHANGE something on the Page. Declared in their own
    // field, deliberately, rather than added to readScopes: the test
    // that forbids a write-shaped scope guards readScopes, and it must
    // go on guarding exactly that. Holding these is not permission to
    // use them. Nothing autonomous can, and lib/workspace/social/
    // mutations.js will not act without an approval a named person has
    // granted plus ENABLE_SOCIAL_MUTATIONS.
    //
    // Enabled on the Meta app "Arrington Consultancy Social" and
    // recorded here on Tom's instruction of 03/09/2026. This is a
    // material expansion of the connector's technical capability and
    // belongs at Governance before it is used against the live Page.
    mutationScopes: ['pages_manage_posts', 'pages_manage_engagement', 'pages_manage_metadata'],
  ...
  instagram: {
    ...
    // instagram_manage_comments is deliberately NOT requested. Governance
    // finding F5 (30/08/2026): it confers comment moderation, including
    // replying and deleting, which is exactly the kind of power this
    // registry's own rule says a token must not carry when the code
    // refuses to use it. Reading comments may be limited as a result,
    // and that is the accepted cost. Confirm against
    // developers.facebook.com/docs/permissions when the Meta app is
    // created.
    // instagram_manage_insights IS requested, and its name is misleading:
    // it is Meta's read-only metrics scope for professional accounts, and
    // there is no instagram_read_insights to use instead. It is the only
    // manage-named scope this registry holds, and test/workspace/social
    // requires that exception to be justified by name rather than let
    // through by a loose pattern.
    readScopes: ['instagram_basic', 'instagram_manage_insights'],
    // Deliberately empty. The Instagram use case is configured for
    // reading only, and nothing here should imply otherwise: an empty
    // list is a statement, where an absent field would be an oversight.
    mutationScopes: [],
    capabilities: AUTONOMOUS_CAPABILITIES,
    supports: { posts: true, metrics: true, comments: false, messages: false, followers: true },
    setupNote: 'Requires an Instagram Business or Creator account linked to the Facebook Page. Personal accounts cannot be read. Comments are not read: the only Meta scope that exposes them also confers moderation, which this connector refuses to hold.'
  },
  linkedin: {
    id: 'linkedin',
    name: 'LinkedIn',
...
// One boot line saying, per platform, whether this deployment actually
// carries the credentials it needs.
//
// Added 03/09/2026, after Tom pasted four Meta credentials into Railway
// and there was no way to tell from the log whether the app had picked
// them up: every other subsystem here prints a status line at boot and
// this one never did, so the only way to find out was to open the page.
//
// It reports PRESENCE and LENGTH only, never any part of a value, which
// is the same discipline the rest of the boot sequence follows and the
// reason it exists: a variable that is visibly correct in a dashboard
// and empty in the container is a real failure mode this project has
// already lost a session to.
//
// It says nothing about whether a credential WORKS. A token can be
// present, well-formed and rejected by Meta, and this line would still
// say configured. That distinction is the connector layer's own rule
// (a credential is never presented as a retrieval), so the wording here
// stops at "the app can see it".
function describeSocialStatus(env = process.env) {
  return PLATFORM_IDS.map((id) => {
    const p = PLATFORMS[id];
    const missing = p.credentialEnv.filter((k) => !(env[k] && String(env[k]).trim()));
    if (!missing.length) {
      const lengths = p.credentialEnv.map((k) => `${k} ${String(env[k]).trim().length} chars`).join(', ');
      return `${p.name}: credentials present (${lengths}), not yet proven to work`;
    }
    if (missing.length === p.credentialEnv.length) return `${p.name}: not connected`;
    return `${p.name}: INCOMPLETE, missing ${missing.join(' and ')}`;
  }).join(' | ');
}
