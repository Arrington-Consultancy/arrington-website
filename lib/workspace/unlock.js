...
// Governance finding G7 (31/08/2026): this used to test v.trim() and
// then return the UNTRIMMED value, so a trailing newline on the Railway
// variable made the passphrase unusable while the boot line reported it
// correctly set. This repository has a documented history with exactly
// that failure: a Railway dashboard form storing a trailing newline cost
// an entire session on the Market Ready Test, and that incident is the
// reason this module prints a length at all. Trimming once, here, means
...
function describeUnlockConfig() {
  ...
  if (trimmed.length < 12) {
    return { ok: false, detail: `WORKSPACE_ACCESS_PASSPHRASE is only ${trimmed.length} characters; 12 is the minimum` };
  }
  // Reports the TRIMMED length, and says so when the stored value
  // carried surrounding whitespace, because that is the difference
  // between "the variable is fine" and "the variable looks fine in the
  // dashboard and does not work".
  const note = trimmed.length === v.length
    ? ''
    : ` (stored value carries ${v.length - trimmed.length} character(s) of surrounding whitespace, which are ignored)`;
  return { ok: true, detail: `WORKSPACE_ACCESS_PASSPHRASE set, length ${trimmed.length}${note}` };
}
