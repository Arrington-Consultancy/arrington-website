...
// WHICH CLOCK DECIDES, stated precisely because the looser version of
// this sentence was itself a finding (Q2).
//
// The AUTHORITATIVE gate is the conditional INSERT in
// claimAlertSlotLocked: its windows are SQL expressions against now(),
// so the database's clock alone decides whether a notice may be
// claimed. Nothing can be written past it.
//
// decideAlert below still compares in JavaScript, against the `now` its
// caller passed. That is deliberate and it is not the gate: it produces
// the human-readable reason, and it is a pure function so that the rule
// can be tested without a database. Where the two clocks disagree, the
// SQL wins, and the only visible consequence is a reason string that
// may be a minute out. The future-dating guard below is what stops that
// skew turning into silence.
//
// A timestamp more than a minute ahead of now cannot be a real recent
// event; it is clock skew or a bad row. Treating it as recent is how a
// future-dated claim silenced the alarm for the whole of the skew
// (finding N4). Ignored rather than trusted, because for an ALARM the
// safe direction is to notify.
function notInTheFuture(at, now) {
  if (!at) return null;
  return (new Date(at).getTime() - now) > 60000 ? null : at;
}
...
function decideAlert({ failuresInWindow, lastSuccessAt, lastFailureAt, lastErrorAt = null, lastPendingAt = null, now, threshold = THRESHOLD, cooldownMinutes = COOLDOWN_MINUTES, failureRetryMinutes = FAILURE_RETRY_MINUTES, claimLeaseMinutes = CLAIM_LEASE_MINUTES }) {
  lastSuccessAt = notInTheFuture(lastSuccessAt, now);
  lastFailureAt = notInTheFuture(lastFailureAt, now);
  lastErrorAt = notInTheFuture(lastErrorAt, now);
  lastPendingAt = notInTheFuture(lastPendingAt, now);
...
async function claimAlertSlotLocked(db, { username, now }) {
  ...
  // Finding P3: this window was still computed from the Node clock while
  // every other one moved to the database's, so the claim that "every
  // authoritative window is now in SQL against now()" was untrue of the
  // one that decides whether a burst counts at all. One clock.
  const { rows: failures } = await db.query(
    `SELECT created_at FROM workspace_activity
      ...
      ORDER BY created_at`,
    [FAILED_EVENT, username, String(WINDOW_MINUTES)]
  );
  const failuresInWindow = failures.length;

  // Retire a claim whose process died, and a claim dated in the future.
  //
  // Finding N4: the age of a claim was computed from the Node clock
  // against a timestamp written by the DATABASE clock. Those are two
  // clocks, and they demonstrably disagree here - a claim seconds old
  // has been observed reading as a minute in the future. A future-dated
  // claim is newer than any lease, so it was never reclaimed and never
  // expired: it silenced the alarm for the whole of the skew, and a
  // badly wrong clock would have silenced it indefinitely.
  //
  // Both windows are now expressed in SQL against now(), so one clock
  // decides, and an implausibly future-dated claim is retired rather
  // than trusted.
  await db.query(
    `UPDATE workspace_activity
        SET event_type = $1,
            summary = 'Security notice was claimed but never completed; the process handling it did not return, or it was dated in the future. Reclaimed.'
      WHERE subject = $2 AND event_type = $3
        AND (created_at < now() - ($4 || ' minutes')::interval
          OR created_at > now() + interval '1 minute')`,
    [ALERT_ABANDONED_EVENT, username, ALERT_PENDING_EVENT, String(CLAIM_LEASE_MINUTES)]
  );
      ...
      `INSERT INTO workspace_activity (actor, event_type, summary, subject)
       SELECT 'system', $1::varchar,
        ...
        WHERE NOT EXISTS (
          SELECT 1 FROM workspace_activity
           WHERE subject = $2
             AND ( (event_type = $3 AND created_at >= now() - ($6 || ' minutes')::interval)
                OR (event_type = $4 AND created_at >= now() - ($7 || ' minutes')::interval)
                OR (event_type = $8 AND created_at >= now() - ($7 || ' minutes')::interval)
                OR (event_type = $5 AND created_at >= now() - ($9 || ' minutes')::interval) )
             AND created_at <= now() + interval '1 minute'
        )
       RETURNING id`,
      [
...
async function maybeAlertOnFailedUnlock(db, { username, now = Date.now(), sendFn } = {}) {
  ...
  try {
    ...
    // Recorded BEFORE the send so the catch below can tell the three
    // cases apart. Governance finding N1 (31/08/2026): without this, any
    // failure after the send - including the single statement that
    // records the outcome - was reported as "NO send was attempted",
    // about a message that had genuinely reached the mailbox. That is
    // the same rule-4 dishonesty M2 corrected, in the opposite
    // direction, introduced by M2's own fix. It also started no
    // cooldown, so a duplicate notice followed five minutes later.
    sendAttempted = true;
    ...
    // What actually happened decides what is recorded, not where the
    // error was thrown (finding N1). Three cases, and only the first may
    // ever say nothing was sent:
    //   - no send attempted            -> error, no cooldown
    //   - attempted and it failed      -> failed send, short backoff
    //   - attempted and it SUCCEEDED   -> delivered, and the hour starts,
    //     because a message that reached the mailbox is delivered
    //     whatever went wrong while recording it. Reporting it as
    //     anything else both lies and produces a duplicate.
    // Finding N3: losing a race is not a failure of the alarm. This was
    // declared as its own error class and then handled exactly like a
    // database fault, so it bought the five-minute send backoff and
    // silenced a genuine burst - the opposite of the reasoning applied
    // to an abandoned claim eleven lines away, which deliberately does
    // NOT gate. Contention is recorded, for the same reason abandonment
    // is, and gates nothing.
    let recorded = false;
    const contended = err instanceof ClaimContentionError;
    const delivered = sendAttempted && sendResult && sendResult.sent === true;
    const outcomeEvent = delivered
      ? ALERT_EVENT
      : (contended ? ALERT_ABANDONED_EVENT : (sendAttempted ? ALERT_FAILED_EVENT : ALERT_ERROR_EVENT));
    const outcomeSummary = delivered
      ? `Security notice DELIVERED for "${username}", but recording the outcome failed: ${err.message}. The notice was sent; the quiet period applies.`
      : (contended
        ? `Security notice for "${username}" stood down under contention: ${err.message}. Nothing was sent and nothing is held back; the next failed attempt decides again.`
        : sendAttempted
          ? `Security notice FAILED to send for "${username}": ${err.message}. This did not start the quiet period; the next attempt will retry.`
          : `Security notice could not be completed for "${username}" and NO send was attempted: ${err.message}`);

    try {
      if (claimId !== null) {
        await db.query(
          'UPDATE workspace_activity SET event_type = $1, summary = $2 WHERE id = $3',
          [outcomeEvent, outcomeSummary, claimId]
        );
      } else {
        // Governance finding P1 (31/08/2026): this branch used to
        // hard-code ALERT_ERROR_EVENT and write its own sentence,
        // ignoring the outcome computed above. Contention is thrown from
        // claimAlertSlot, which is awaited BEFORE claimId is assigned, so
        // a contended failure always lands here - which made the N3 fix
        // dead code, recorded as corrected in the remediation, in a
        // comment, and in CLAUDE.md, while the original defect stood.
        //
        // Both branches now record the same decision. There is one
        // outcome, so there is one place that decides it.
        await db.query(
          'INSERT INTO workspace_activity (actor, event_type, summary, subject) VALUES ($1,$2,$3,$4)',
          ['system', outcomeEvent, outcomeSummary, username]
        );
      }
      recorded = true;
    } catch (inner) {
    ...
    // Report what happened, not merely that something went wrong. A
    // delivered notice is delivered even when recording it failed
    // (finding N1); saying otherwise here would put the same untruth in
    // the caller's hands that it put in the register.
    // Finding Q4: recordedAs used to name the row this code INTENDED to
    // write, whether or not the write succeeded. When the database is
    // what failed, that is a claim about a row that does not exist -
    // the same class of untruth as N1, one layer out. It now reports
    // null when nothing was recorded.
    return {
      sent: Boolean(delivered),
      error: err.message,
      recordedAs: recorded ? outcomeEvent : null
    };
  }
}
