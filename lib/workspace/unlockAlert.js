...
// Wins the right to send, or returns null. The INSERT ... SELECT ...
// WHERE NOT EXISTS is evaluated by the database, so two concurrent
// callers cannot both succeed: one inserts, the other's NOT EXISTS is
     ...
     SELECT 'system', $1::varchar,
      ...
      WHERE NOT EXISTS (
        SELECT 1 FROM workspace_activity
         WHERE subject = $2
           AND ( (event_type = $3 AND created_at >= $5)
              OR (event_type = $4 AND created_at >= $6)
              OR (event_type = $1 AND created_at >= $7) )
      )
     RETURNING id`,
    [
      ALERT_PENDING_EVENT, username, ALERT_EVENT, ALERT_FAILED_EVENT,
...
async function maybeAlertOnFailedUnlock(db, { username, now = Date.now(), sendFn } = {}) {
  ...
  try {
    ...
    if (failures.length < THRESHOLD) {
      return { sent: false, quiet: true, reason: `${failures.length} failure(s) in the window, threshold is ${THRESHOLD}` };
    }

    // Everything after this point is guarded by winning the claim.
    claimId = await claimAlertSlot(db, { username, now });
    if (claimId === null) {
      return { sent: false, quiet: true, reason: 'another attempt is already sending, or a recent notice or failure covers this burst' };
    }
